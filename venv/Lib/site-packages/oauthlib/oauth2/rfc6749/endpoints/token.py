...
class TokenEndpoint(BaseEndpoint):
    ...
    The token endpoint is used by the client to obtain an db token by
    presenting its authorization grant or refresh token.  The token
    endpoint is used with every authorization grant except for the
    implicit grant type (since an db token is issued directly).
    ...
    The client MUST use the HTTP "POST" method when making db token
    requests::
