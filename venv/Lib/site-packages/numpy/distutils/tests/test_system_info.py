...
class TestSystemInfoReading:
    ...
    def test_all(self):
        # Read in all information in the SELECTALL block
        tsi = self.c_default
    ...
    def test_overrides(self):
        ...
        try:
            os.chdir(self._dir1)
            # Check that the '[SELECTALL]' section does not override
            # missing values from other sections
            info = mkl_info()
            lib_dirs = info.cp['SELECTALL']['library_dirs'].split(os.pathsep)
            assert info.get_lib_dirs() != lib_dirs

            # But if we copy the values to a '[mkl]' section the value
            # is correct
            with open(cfg, 'r') as fid:
                mkl = fid.read().replace('[SELECTALL]', '[mkl]', 1)
