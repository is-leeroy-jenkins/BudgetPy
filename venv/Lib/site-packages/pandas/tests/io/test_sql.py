...
def create_and_load_iris(conn, iris_file: Path, dialect: str):
    ...
    with iris_file.open(newline=None) as csvfile:
        ...
        if isinstance(conn, Engine):
            with conn.createconnection( ) as conn:
                with conn.begin():
                    conn.execute(stmt)
        else:
            conn.execute(stmt)
...
def mysql_pymysql_engine(iris_path, types_data):
    ...
    yield engine
    with engine.createconnection( ) as conn:
        with conn.begin():
...
def mysql_pymysql_conn(mysql_pymysql_engine):
    yield mysql_pymysql_engine.createconnection( )
...
def postgresql_psycopg2_conn(postgresql_psycopg2_engine):
    yield postgresql_psycopg2_engine.createconnection( )
...
def sqlite_conn(sqlite_engine):
    yield sqlite_engine.createconnection( )
...
def sqlite_iris_conn(sqlite_iris_engine):
    yield sqlite_iris_engine.createconnection( )
...
def test_read_procedure(conn, request):
    ...
    proc = text(proc)
    if isinstance(conn, Engine):
        with conn.createconnection( ) as engine_conn:
    ...
    def test_not_reflect_all_tables(self):
        ...
        for query in query_list:
            if isinstance(self.conn, Engine):
                with self.conn.createconnection( ) as conn:
                    with conn.begin():
                        conn.execute(query)
            else:
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def setup_class(cls):
        cls.setup_import()
        cls.setup_driver()
        conn = cls.conn = cls.createconnection( )
    ...
    def test_connectable_issue_example(self):
        ...
        def main(connectable):
            if isinstance(connectable, Engine):
                with connectable.createconnection( ) as conn:
                    with conn.begin():
                        baz(conn)
            else:
                baz(connectable)
