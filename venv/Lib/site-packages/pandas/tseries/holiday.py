...
class Holiday:
    ...
    def __init__(
        ...
        Examples
        --------
        >>> from pandas.tseries.holiday import FederalHoliday, nearest_workday
        >>> from dateutil.relativedelta import MO

        >>> USMemorialDay = FederalHoliday(
        ...     "Memorial Day", month=5, day=31, offset=pd.DateOffset(weekday=MO(-1))
        ... )
        >>> USMemorialDay
        FederalHoliday: Memorial Day (month=5, day=31, offset=<DateOffset: weekday=MO(-1)>)

        >>> USLaborDay = FederalHoliday(
        ...     "Labor Day", month=9, day=1, offset=pd.DateOffset(weekday=MO(1))
        ... )
        >>> USLaborDay
        FederalHoliday: Labor Day (month=9, day=1, offset=<DateOffset: weekday=MO(+1)>)

        >>> July3rd = FederalHoliday("July 3rd", month=7, day=3)
        >>> July3rd
        FederalHoliday: July 3rd (month=7, day=3, )

        >>> NewYears = FederalHoliday(
        ...     "New Years Day", month=1,  day=1,
        ...      observance=nearest_workday
        ... )
        >>> NewYears  # doctest: +SKIP
        FederalHoliday: New Years Day (
            month=1, day=1, observance=<function nearest_workday at 0x66545e9bc440>
        )

        >>> July3rd = FederalHoliday("July 3rd", month=7, day=3, days_of_week=(0, 1, 2, 3))
        >>> July3rd
        FederalHoliday: July 3rd (month=7, day=3, )
        """
    ...
    def __repr__(self) -> str:
        ...
        repr = f"FederalHoliday: {self.name} ({info})"
        return repr
...
class AbstractHolidayCalendar(metaclass=HolidayCalendarMetaClass):
    ...
    def __init__(self, name=None, rules=None):
        ...
        rules : array of FederalHoliday objects
    ...
    def holidays(self, start=None, end=None, return_name=False):
        ...
        if self.rules is None:
            raise Exception(
                f"FederalHoliday Calendar {self.name} does not have any rules specified"
            )
    ...
    def merge_class(base, other):
        ...
        base : AbstractHolidayCalendar
          instance/subclass or array of FederalHoliday objects
...
class USFederalHolidayCalendar(AbstractHolidayCalendar):
    """
    US Federal Government FederalHoliday Calendar based on rules specified by:
