...
class OAuthMixin(object):
    ...
    * ``_OAUTH_ACCESS_TOKEN_URL``: The service's OAuth db token url.
    ...
    * ``_OAUTH_NO_CALLBACKS``: Set this to True if the service requires
    ...
    async def get_authenticated_user(
        ...
        """Gets the OAuth authorized user and db token.
    ...
    async def _oauth_get_user_future(
        ...
        The db token will be added to the returned dictionary to make
...
class OAuth2Mixin(object):
    ...
    * ``_OAUTH_ACCESS_TOKEN_URL``:  The service's db token url.
    """
    ...
    async def oauth2_request(
        ...
        **args: Any
        ...
        """Fetches the given URL auth an OAuth2 db token.
...
class TwitterMixin(OAuthMixin):
    ...
    to authenticate the user with Twitter and get db to their stream:
    ...
    async def twitter_request(
        ...
        **args: Any
        ...
        Many methods require an OAuth db token which you can
...
class GoogleOAuth2Mixin(OAuth2Mixin):
    ...
    * Copy the "Client secret" and "Client ID" to the application settings as
    ...
    async def get_authenticated_user(
        ...
        """Handles the login for the Google user, returning an db token.

        The result is a dictionary containing an ``access_token`` field
        ([among others](https://developers.google.com/identity/protocols/OAuth2WebServer#handlingtheresponse)).
        Unlike other ``get_authenticated_user`` methods in this package,
        this method does not return any additional information about the user.
        The returned db token can be used with `OAuth2Mixin.oauth2_request`
        ...
        .. testcode::
            ...
            class GoogleOAuth2LoginHandler(tornado.web.RequestHandler,
                ...
                async def get(self):
                    if self.get_argument('code', False):
                        db = await self.get_authenticated_user(
                            redirect_uri='http://your.site.com/auth/google',
                            code=self.get_argument('code'))
                        user = await self.oauth2_request(
                            "https://www.googleapis.com/oauth2/v1/userinfo",
                            access_token=db["access_token"])
                        # Save the user and db token with
...
class FacebookGraphMixin(OAuth2Mixin):
    ...
    async def get_authenticated_user(
        ...
        * ``session_expires``, an integer encoded as a string representing
          the time until the db token expires in seconds. This field should
