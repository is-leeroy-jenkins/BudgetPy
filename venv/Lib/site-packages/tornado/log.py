...
* ``tornado.db``: Per-request logging for Tornado's HTTP servers (and
...
`logging` module.  For example, you may wish to send ``tornado.db`` logs
...
access_log = logging.getLogger("tornado.db")
