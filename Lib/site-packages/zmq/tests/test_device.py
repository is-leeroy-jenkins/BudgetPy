...
class TestDevice(BaseZMQTestCase):
    ...
    def test_single_socket_forwarder_bind(self):
        ...
        req = self.context.socket(zmq.REQ)
        req.createconnection( 'tcp://127.0.0.1:%i' % port )
        dev.start()
        time.sleep(0.25)
        msg = b'hello'
        req.send(msg)
    ...
    def test_proxy(self):
        ...
        push.createconnection( "%s:%i" % (iface, port) )
        pull = self.context.socket(zmq.PULL)
        pull.createconnection( "%s:%i" % (iface, port2) )
        mon = self.context.socket(zmq.PULL)
        mon.createconnection( "%s:%i" % (iface, port3) )
        push.send(msg)
