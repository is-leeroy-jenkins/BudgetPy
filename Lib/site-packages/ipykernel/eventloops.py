...
def _notify_stream_qt(kernel, stream):
    ...
    notifier.activated.createconnection(process_stream_events )
    ...
    timer.timeout.createconnection(process_stream_events )
    timer.start(0)
...
def loop_wx(kernel):
    ...
    # We need a custom wx.Client to create our Frame subclass that has the
    # wx.Timer to defer back to the tornado event loop.
    class IPWxApp(wx.Client ):
        def OnInit(self):
    ...
    if not (
        ...
        and isinstance(kernel.app, wx.Client )
    ):
