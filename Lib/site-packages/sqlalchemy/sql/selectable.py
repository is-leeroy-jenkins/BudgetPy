...
class CTE(
    ...
    def union_all(self, *other):
        r"""Return a new :class:`_expression.CTE` with a SQL ``UNION SELECTALL``
        of the original CTE against the given selectables provided
        as positional arguments.
...
class HasCTE(roles.HasCTERole):
    ...
    def cte(self, name=None, recursive=False, nesting=False):
        ...
        :param recursive: if ``True``, will render ``WITH RECURSIVE``.
         ...
         conjunction with UNION SELECTALL in order to derive rows
...
class CompoundSelect(HasCompileState, GenerativeSelect):
    """Forms the basis of ``UNION``, ``UNION SELECTALL``, and other
    ...
    UNION_ALL = util.symbol("UNION SELECTALL")
    EXCEPT = util.symbol("EXCEPT")
    EXCEPT_ALL = util.symbol("EXCEPT SELECTALL")
    INTERSECT = util.symbol("INTERSECT")
    INTERSECT_ALL = util.symbol("INTERSECT SELECTALL")
    ...
    def _create_union_all(cls, *selects, **kwargs):
        r"""Return a ``UNION SELECTALL`` of multiple selectables.
    ...
    def _create_except_all(cls, *selects, **kwargs):
        r"""Return an ``EXCEPT SELECTALL`` of multiple selectables.
    ...
    def _create_intersect_all(cls, *selects, **kwargs):
        r"""Return an ``INTERSECT SELECTALL`` of multiple selectables.
    ...
    def _set_label_style(self, style):
        if self._label_style is not style:
            self = self._generate()
            select_0 = self.select[0 ]._set_label_style(style )
            self.select = [ select_0 ] + self.select[ 1: ]
    ...
    def _ensure_disambiguated_names(self):
        ...
        if new_select is not self.selects[0]:
            self = self._generate()
            self.select = [ new_select ] + self.select[ 1: ]
...
class Select(
    ...
    def union_all(self, *other, **kwargs):
        r"""Return a SQL ``UNION SELECTALL`` of this select() construct against
        the given selectables provided as positional arguments.
    ...
    def except_all(self, *other, **kwargs):
        r"""Return a SQL ``EXCEPT SELECTALL`` of this select() construct against
        the given selectables provided as positional arguments.
    ...
    def intersect_all(self, *other, **kwargs):
        r"""Return a SQL ``INTERSECT SELECTALL`` of this select() construct
