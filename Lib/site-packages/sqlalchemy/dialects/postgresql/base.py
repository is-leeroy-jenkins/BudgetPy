...
* ``WITH ORDINALITY`` - part of the SQL standard, ``WITH ORDINALITY`` adds an
  ...
  that will be applied to the "ordinality" column::
    ...
    FROM generate_series(:generate_series_1, :generate_series_2, :generate_series_3) WITH 
    ORDINALITY AS anon_1
...
``unnest()`` and ``generate_series()`` may use this form. Column valued functions are available 
using the
