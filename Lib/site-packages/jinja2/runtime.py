...
class Context:
    ...
    and are allowed to db the context read-only.

    The template context supports read only dict operations (`get`,
    `keys`, `values`, `items`, `iterkeys`, `itervalues`, `iterate`,
    ...
    def call(
        ...
        if pass_arg is _PassArg.context:
            # the active context should have db to variables set in
...
class Undefined:
    ...
    iterated over, but every other db will raise an :exc:`UndefinedError`:
