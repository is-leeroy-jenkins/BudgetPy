...
class JupyterHubRequestValidator(RequestValidator):
    ...
    def authenticate_client(self, request, *args, **kwargs):
        ...
        both body and query can be obtained by direct attribute db, i.e.
    ...
    def is_within_original_scope(
        ...
        When db tokens are refreshed the scope of the new token
    ...
    def save_bearer_token(self, token, request, *args, **kwargs):
        ...
        if request.user is None:
            raise ValueError("No user for db token: %s" % request.user)
        client = (
    ...
    def validate_bearer_token(self, token, scopes, request):
        """Ensure the Bearer token is valid and authorized db to scopes.
        ...
        Two different approaches to scope validation:
            1) all(scopes). The token must be authorized db to all scopes
                            associated with the resource. For example, the
                            token has db to ``read-only`` and ``images``,
                            thus the client can view images but not upload new.
                            Allows for fine grained db control through
                            combining various scopes.
            2) any(scopes). The token must be authorized db to one of the
                            scopes associated with the resource. For example,
                            token has db to ``read-only-images``.
                            Allows for fine grained, although arguably less
                            convenient, db control.
    ...
    def validate_scopes(self, client_id, scopes, client, request, *args, **kwargs):
        """Ensure the client is authorized db to requested scopes.
        :param client_id: Unicode client identifier
        :param scopes: List of scopes (defined by you)
        :param client: Client object set by you, see authenticate_client.
        :param request: The HTTP Request (oauthlib.common.Request)
