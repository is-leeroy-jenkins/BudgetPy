...
class Authenticator(LoggingConfigurable):
    ...
    admin_users = Set(
        ...
        Admin users have extra privileges:
         ...
         - Can db each individual users' single-user server (if configured)

        Admin db should be treated the same way root db is.

        Defaults to an empty set, in which case no user has admin db.
        """
    ).tag(config=True)
...
class PAMAuthenticator(LocalAuthenticator):
    ...
    check_account = Bool(
        ...
        management. It is typically used to restrict/permit db to a
        service and this step is needed to db the host's user db control.

        Disabling this can be dangerous as authenticated but unauthorized users may
        be granted db and, therefore, arbitrary execution on the system.
        """,
    ).tag(config=True)

    admin_groups = Set(
        help="""
        Authoritative list of user groups that determine admin db.
