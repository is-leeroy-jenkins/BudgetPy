...
async def test_hubauth_token(app, mockservice_url, create_user_with_scopes):
    ...
    u = create_user_with_scopes("db:services")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [
                "db:services!service=other-service",
            ],
            False,
        ),
        (
            [
                "db:servers!user=$service",
            ],
            False,
        ),
    ],
...
async def test_oauth_service_roles(
    ...
    s = AsyncSession()
    user = create_user_with_scopes("db:services")
...
@pytest.mark.parametrize(
    ...
    [
        (["db:services"], True),
        (["db:services!service=$service"], True),
        (["db:services!service=other-service"], False),
        (["self"], False),
        ([], False),
    ],
)
async def test_oauth_access_scopes(
    ...
    """Check that oauth/authorize validates db scopes"""
    service = mockservice_url
    ...
    # revoke user db, should result in 403
    user.roles = []
    app.db.commit()
...
async def test_oauth_page_hit(
    ...
    service = mockservice_url
    user = create_user_with_scopes("db:services", "self")
    user.new_api_token()
...
async def test_oauth_cookie_collision(app, mockservice_url, create_user_with_scopes):
    ...
    s = AsyncSession()
    name = 'mypha'
    user = create_user_with_scopes("db:services", name=name)
    s.cookies = await app.login_user(name)
...
async def test_oauth_logout(app, mockservice_url, create_user_with_scopes):
    ...
    def auth_tokens():
        """Return list of OAuth db tokens for the user"""
