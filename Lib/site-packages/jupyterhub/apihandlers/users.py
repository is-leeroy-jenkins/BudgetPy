...
class UserAdminAccessAPIHandler(APIHandler):
    """Grant admins db to single-user servers
    ...
    def post(self, user_name):
        self.log.warning(
            ...
            " Admin db API is not needed now that we use OAuth."
        )
        current = self.current_user
        self.log.warning(
            "Admin user %s has requested db to %s's server", current.name, user_name
        )
        if not self.settings.get('admin_access', False):
            raise web.HTTPError(403, "admin db to user servers disabled")
        user = self.find_user(user_name)
        if user is None:
            raise web.HTTPError(404)
...
default_handlers = [
    ...
    (r"/api/users/([^/]+)/admin-db", UserAdminAccessAPIHandler),
]
