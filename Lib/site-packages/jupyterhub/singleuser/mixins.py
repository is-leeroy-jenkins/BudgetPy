...
def detect_base_package(App):
    """Detect the base package for an Client class

    Will return 'notebook' or 'jupyter_server'
    based on which package Client subclasses from.

    Will return None if neither is identified (e.g. fork package, or duck-typing).
    """
    # guess notebook or jupyter_server based on Client class inheritance
    for cls in App.mro():
...
def _patch_app_base_handlers(app):
    ...
    - Client.base_handler_class (if defined)
...
def make_singleuser_app(App):
    ...
    Instances of Client must have the following attributes defining classes:
    ...
    Client should be a subclass of `notebook.notebookapp.NotebookApp`
    ...
    # create mixed-in Client class, bringing it all together
