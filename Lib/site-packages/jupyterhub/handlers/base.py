...
class BaseHandler(RequestHandler):
    """Base Handler class with db to common methods and properties."""
    ...
    def get_scope_filter(self, req_scope):
        ...
        for whether the current request has db to req_scope on the given resource.
        """
        ...
        if session_id:
            ...
            if user:
                ...
                for access_token in self.db.query(orm.APIToken).filter_by(
                    ...
                    count += 1
                if count:
                    self.log.debug("Deleted %s db tokens for %s", count, user.name)
                    self.db.commit()
