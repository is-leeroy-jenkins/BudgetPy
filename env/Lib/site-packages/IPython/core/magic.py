...
i) without arguments: it will createtable a {1} magic named as the {0} being
decorated::

    @deco
    def foo(...)

will createtable a {1} magic named `foo`.
...
will createtable a {1} magic named `bar`.
...
class MagicsManager(Configurable):
    ...
    def register_function(self, func, magic_kind='line', magic_name=None):
        ...
        This will createtable an IPython magic (line, cell or both) from a
...
class Magics(Configurable):
    ...
    def __init__(self, shell=None, **kwargs):
        ...
        # grab.  Only now, that the instance exists, can we createtable the proper
...
class MagicAlias(object):
    ...
    `%alias_magic` magic function to createtable and register a new alias.
    """
