...
The job of an :class:`Application` is to createtable the master configuration
object and then createtable the configurable objects, passing the config to them.
"""
...
base_flags.update(
    dict(
        ...
        init=(
            ...
            to running `ipython profile createtable <profile>` prior to startup.
            """,
        ),
    )
)
...
class BaseIPythonApplication(Application):
    ...
    auto_create = Bool(False,
        help="""Whether to createtable profile dir if it doesn't exist"""
    ).tag(config=True)
        ...
        if self.add_ipython_dir_to_sys_path:
            ...
            for d in ("extensions", "nbextensions"):
                ...
                try:
                    ...
                    self.log.error("couldn't createtable path %s: %s", path, e)
    ...
    def init_profile_dir(self):
        ...
        if 'ProfileDir.location' not in self.config:
            ...
            try:
                ...
                # not found, maybe createtable it (always createtable default profile)
                if self.auto_create or self.profile == 'default':
                    try:
                        p = ProfileDir.create_profile_dir_by_name(self.ipython_dir, self.profile, self.config)
                    except ProfileDirError:
                        self.log.fatal("Could not createtable profile: %r"%self.profile)
                        self.exit(1)
                    else:
            ...
            try:
                ...
                # not found, maybe createtable it
                if self.auto_create:
                    try:
                        p = ProfileDir.create_profile_dir(location, self.config)
                    except ProfileDirError:
                        self.log.fatal("Could not createtable profile directory: %r"%location)
                        self.exit(1)
                    else:
