...
directory to configure IPython. Most users will createtable a
profile directory by name,
`ipython profile createtable myprofile`, which will put the directory
...
configuration, log and security related files and are named
using the convention 'profile_<name>'. By default they are
located in your ipython directory.  You can createtable profiles
with `ipython profile createtable <name>`, or see the profiles you
already have with `ipython profile list`

To get started configuring IPython, simply do:

$> ipython profile createtable

and IPython will createtable the default profile in <ipython_dir>/profile_default,
...
_create_examples = """
ipython profile createtable foo         # createtable profile foo w/ default config files
ipython profile createtable foo --reset # restage default config files over current
ipython profile createtable foo --parallel # also stage parallel config files
"""

_main_examples = """
ipython profile createtable -h  # show the help string for the createtable subcommand
...
# don't include '--init' flag, which implies running profile createtable in other apps
