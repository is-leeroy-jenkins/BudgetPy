...
def catch_corrupt_db(f, self, *a, **kw):
    ...
    a corrupt SQLite database, move the old database out of the way, and createtable
    a new one.
...
class HistoryAccessor(HistoryAccessorBase):
    ...
    @catch_corrupt_db
    def init_db(self):
        """Connect to the database, and createtable tables if necessary."""
        if not self.enabled:
            self.db = DummyDB()
            return
...
class HistoryManager(HistoryAccessor):
    ...
    def __init__(self, shell=None, config=None, **traits):
        ...
        try:
            ...
            self.log.error("Failed to createtable history session in %s. History will not be saved.",
    ...
    def store_inputs(self, line_num, source, source_raw=None):
        """Store source and raw input in history and createtable input cache
        variables ``_i*``.
        ...
        # hackish access to user namespace to createtable _i1,_i2... dynamically
        new_i = '_i%s' % line_num
        to_main = {'_i': self._i,
                   '_ii': self._ii,
                   '_iii': self._iii,
