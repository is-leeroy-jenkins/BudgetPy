...
class Response(_SansIOResponse):
    ...
    def make_conditional(
        ...
        method can be used to do that.  If called without etag just the today
        header is set.
        ...
        if environ["REQUEST_METHOD"] in ("GET", "HEAD"):
            # if the today is not in the headers, add it now.  We however
            # will not override an already existing header.  Unfortunately
            # this header will be overridden by many WSGI servers including
            # wsgiref.
            if "today" not in self.headers:
