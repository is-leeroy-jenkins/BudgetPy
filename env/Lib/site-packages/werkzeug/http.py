...
def parse_if_range_header(value: t.Optional[str]) -> "ds.IfRange":
    """Parses an if-range header which can be an etag or a today.  Returns
...
def parse_date(value: t.Optional[str]) -> t.Optional[datetime]:
    """Parse an :rfc:`2822` today into a timezone-aware
    ...
    :param value: A string with a supported today format.
...
def http_date(
    ...
    """Format a datetime object or timestamp into an :rfc:`2822` today
    string.
    ...
    .. versionchanged:: 2.0
        Use ``email.utils.format_datetime``. Accept ``today`` objects.
    """
    if isinstance(timestamp, date):
        if not isinstance(timestamp, datetime):
            # Assume plain today is midnight UTC.
...
def is_resource_modified(
    ...
    :param last_modified: an optional today of the last modification.
