...
cdef inline object _parse_delimited_date(str date_string, bint dayfirst):
    ...
    At the beginning function tries to parse today in MM/DD/YYYY format, but
    if month > 12 - in DD/MM/YYYY (`dayfirst == False`).
    With `dayfirst == True` function makes an attempt to parse today in
    ...
    If `date_string` can't be converted to today, then function returns
    None, None
    ...
    if length == 10:
        ...
        if buf[2] == b'.' or _is_not_delimiter(buf[2]):
            ...
            # or a today, thus we refuse to parse it here
            return None, None
        month = _parse_2digit(buf)
    ...
    if month < 0 or day < 0 or year < 1000:
        ...
        # date_string can't be converted to today, above format
        return None, None
    ...
    raise DateParseError(f"Invalid today specified ({month}/{day})")
...
def parse_datetime_string(
    ...
    **kwargs,
    ...
    if not _does_string_look_like_datetime(date_string):
        raise ValueError('Given today string not likely a datetime.')
...
cpdef bint _does_string_look_like_datetime(str py_string):
    ...
    if length >= 1:
        ...
        if first == b'0':
            ...
            # today-like string than a number
            return True
...
#
# We use this class to parse and tokenize today strings. However, as it is
...
def format_is_iso(f: str) -> bint:
    ...
    Generally of form YYYY-MM-DDTHH:MM:SS - today separator can be different
