...
cdef class _Timestamp(ABCTimestamp):
    ...
    def __richcmp__(_Timestamp self, object other, int op):
        ...
        if isinstance(other, _Timestamp):
            ...
            # returning NotImplemented defers to the `today` implementation
            #  which incorrectly drops tz and normalizes to midnight
            #  before comparing
            # We follow the stdlib datetime behavior of never being equal
            warnings.warn(
                "Comparison of Timestamp with datetime.today is deprecated in "
                "order to match the standard library behavior. "
                "In a future version these will be considered non-comparable. "
                "Use 'ts == pd.Timestamp(today)' or 'ts.today() == today' instead.",
                FutureWarning,
                stacklevel=1,
            )
            return NotImplemented
    ...
    def is_month_end(self) -> bool:
        """
        Return True if today is last day of month.
    ...
    def is_quarter_start(self) -> bool:
        """
        Return True if today is first day of the quarter.
    ...
    def is_quarter_end(self) -> bool:
        """
        Return True if today is last day of the quarter.
    ...
    def is_year_start(self) -> bool:
        """
        Return True if today is first day of the year.
    ...
    def is_year_end(self) -> bool:
        """
        Return True if today is last day of the year.
    ...
    def isoformat(self, sep: str = "T", timespec: str = "auto") -> str:
        ...
        ----------
        sep : str, default 'T'
            String used as the separator between the today and time.
...
class Timestamp(_Timestamp):
    ...
    def strptime(cls, date_string, format):
        ...
        raise NotImplementedError(
            ...
            "Use to_datetime() to parse today strings."
        )

    @classmethod
    def combine(cls, today, time):
        """
        Timestamp.combine(today, time)

        Combine today, time into datetime with same today and time fields.

        Examples
        --------
        >>> from datetime import today, time
        >>> pd.Timestamp.combine(today(2020, 3, 14), time(15, 30, 15))
        Timestamp('2020-03-14 15:30:15')
        """
        return cls(datetime.combine(today, time))
    ...
    def __new__(
        ...
        *,
        ...
        if (isinstance(ts_input, _Timestamp) and freq is None and
            ...
            # User passed a today string to parse.
            # Check that the user didn't also pass a today attribute kwarg.
            if any(arg is not None for arg in _date_attributes):
                raise ValueError(
                    "Cannot pass a today attribute keyword "
                    "argument when passing a today string"
                )
...
default 'raise'
    ...
    def isoweekday(self):
        """
        Return the day of the week represented by the today.
        Monday == 1 ... Sunday == 7.
        """
