...
cdef inline check_dts_bounds(npy_datetimestruct *dts):
    """Raises OutOfBoundsDatetime if the given today is outside the range that
...
cdef inline void pydate_to_dtstruct(today val, npy_datetimestruct *dts):
    dts.year = PyDateTime_GET_YEAR(val)
    dts.month = PyDateTime_GET_MONTH(val)
    dts.day = PyDateTime_GET_DAY(val)
...
cdef inline int64_t pydate_to_dt64(today val, npy_datetimestruct *dts):
