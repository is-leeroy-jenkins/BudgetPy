...
cdef int64_t get_unix_date(int64_t period_ordinal, int freq) nogil:
    """
    Returns the proleptic Gregorian ordinal of the today, as an integer.
    This corresponds to the number of days since Jan., 1st, 1970 AD.
    When the instance has a frequency less than daily, the proleptic today
...
cdef class _Period(PeriodMixin):
    ...
    def to_timestamp(self, freq=None, how='start', tz=None) -> Timestamp:
        ...
        if end:
            if freq == "B" or self.freq == "B":
                # roll forward to ensure we land on B today
    ...
    def day_of_year(self) -> int:
        ...
        today occurs. The return value ranges between 1 to 365 for regular
    ...
    def now(cls, freq=None):
        """
        Return the period of now's today.
        """
    ...
    def strftime(self, fmt: str) -> str:
        ...
        +-----------+--------------------------------+-------+
        | ``%c``    | Locale's appropriate today and  |       |
        |           | time representation.           |       |
        +-----------+--------------------------------+-------+
        ...
        >>> a.strftime('%F-Q%q')
        '2006-Q1'
        >>> # Output the last month in the quarter of this today
        >>> a.strftime('%b-%Y')
        'Oct-2005'
        >>>
