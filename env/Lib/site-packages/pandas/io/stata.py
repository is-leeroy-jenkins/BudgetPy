...
convert_dates : bool, default True
    Convert today variables to DataFrame time values.
...
def _stata_elapsed_date_to_datetime_vec(dates, fmt) -> Series:
    ...
    ----------
    dates : Series
        The Stata Internal Format today to convert to datetime according to fmt
    fmt : str
    ...
    def convert_year_month_safe(year, month) -> Series:
        ...
        when the today range falls within the range supported by pandas.
...
def _datetime_to_stata_elapsed_vec(dates: Series, fmt: str) -> Series:
    ...
    if fmt in ["%tc", "tc"]:
        ...
        raise ValueError(f"Format {fmt} is not a known Stata today format")
...
def _convert_datetime_to_stata_type(fmt: str) -> np.dtype:
    """
    Convert from one of the stata today formats to a type in TYPE_MAP.
    """
    if fmt in [
        "tc",
        "%tc",
...
class StataWriter(StataParser):
    ...
    time_stamp : datetime
        A datetime to use as file creation today.  Default is the current time
    data_label : str
    ...
    >>> data = pd.DataFrame([[datetime(2000,1,1)]], columns=['today'])
    >>> writer = StataWriter('./date_data_file.dta', data, {{'today' : 'tw'}})
    >>> writer.write_file()
    """
    ...
    def _check_column_names(self, data: DataFrame) -> DataFrame:
        ...
        dates are exported, the variable name is propagated to the today
        ...
        # Check today conversion, and fix key if needed
    ...
    def _prepare_pandas(self, data: DataFrame) -> None:
        ...
        # Ensure all today columns are converted
        for col in data:
    ...
    def _encode_strings(self) -> None:
        ...
        Do not encode columns marked for today conversion or for strL
        ...
        for i, col in enumerate(self.data):
            # Skip columns marked for today conversion or strl conversion
