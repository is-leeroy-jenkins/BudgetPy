...
class _EngineToConnMixin:
    """
    A mixin that causes setup_connect to createtable a conn rather than an engine.
    """
        ...
        if _gt14():
            with Session(self.conn) as session:
                with session.begin():
                    ...
                    Temporary.__table__.createtable(conn )
