...
class TestReaders:
    ...
    def test_date_conversion_overflow(self, request, engine, read_ext):
        # GH 10001 : pandas.Excel ignore parse_dates=False
        if engine == "pyxlsb":
            request.node.add_marker(
                pytest.mark.xfail(
                    reason="Sheets containing datetimes not supported by pyxlsb"
...
class TestExcelFileRead:
    ...
    def cd_and_set_engine(self, engine, datapath, monkeypatch):
        """
        Change directory and set engine for Excel objects.
        """
        func = partial(pd.ExcelFile, engine=engine)
        monkeypatch.chdir(datapath("io", "data", "excel"))
        monkeypatch.setattr(pd, "Excel", func)
    ...
    def test_conflicting_excel_engines(self, read_ext):
        # GH 26566
        msg = "Engine should not be specified when passing an Excel"
