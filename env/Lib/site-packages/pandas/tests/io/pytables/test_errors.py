...
def test_unimplemented_dtypes_table_columns(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        dtypes = [("today", datetime.date(2001, 1, 2))]
    ...
    with ensure_clean_store(setup_path) as store:
        # this fails because we have a today in the object block......
        msg = re.escape(
            """Cannot serialize the column [datetime1]
because its data contents are not [string] but [today] object dtype"""
        )
        with pytest.raises(TypeError, match=msg):
