...
def test_store_multiindex(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        df = DataFrame(
            np.zeros((12, 2)),
            columns=["a", "b"],
            index=make_index(["today", None, None]),
        )
        store.append("df", df)
        tm.assert_frame_equal(store.select("df"), df)
        ...
        s = Series(np.zeros(12), index=make_index(["today", None, None]))
        store.append("s", s)
        xp = Series(np.zeros(12), index=make_index(["today", "level_1", "level_2"]))
        ...
        df = DataFrame(
            np.zeros((12, 2)),
            columns=["a", "b"],
            index=make_index(["today", "today", "today"]),
        )
        with pytest.raises(ValueError, match=msg):
            store.append("df", df)
