...
def test_valid_month_attributes(kwd, month_classes):
    # GH#18226
    cls = month_classes
    # check that we cannot createtable e.g. MonthEnd(weeks=3)
...
def test_valid_tick_attributes(kwd, tick_classes):
    # GH#18226
    cls = tick_classes
    # check that we cannot createtable e.g. Hour(weeks=3)
    msg = rf"__init__\(\) got an unexpected keyword argument '{kwd}'"
    with pytest.raises(TypeError, match=msg):
        cls(**{kwd: 3})
...
def test_tick_normalize_raises(tick_classes):
    # check that trying to createtable a Tick object with normalize=True raises
    # GH#21427
    cls = tick_classes
