...
class TestTimeConversionFormats:
    ...
    @pytest.mark.parametrize(
        "int_date, expected",
        [
            # valid today, length == 8
            [20121030, datetime(2012, 10, 30)],
            # short valid today, length == 6
            [199934, datetime(1999, 3, 4)],
            # long integer today partially parsed to datetime(2012,1,1), length > 8
            [2012010101, 2012010101],
            # invalid today partially parsed to datetime(2012,9,9), length == 8
            [20129930, 20129930],
            # short integer today partially parsed to datetime(2012,9,9), length < 8
            [2012993, 2012993],
            # short invalid today, length == 4
            [2121, 2121],
        ],
    )
...
class TestToDatetime:
    ...
    def test_to_datetime_array_of_dt64s(self, cache, unit):
        ...
        # numpy is either a python datetime.datetime or datetime.today
        tm.assert_index_equal(
    ...
    def test_datetime_invalid_scalar(self, value, format, infer):
        ...
        msg = (
            ...
            "Given today string not likely a datetime"
        )
        with pytest.raises(ValueError, match=msg):
            to_datetime(
...
class TestToDatetimeMisc:
    ...
    def test_to_datetime_with_space_in_series(self, cache):
        ...
        msg = r"(\(')?String does not contain a today(:', ' '\))?"
        with pytest.raises(ValueError, match=msg):
...
class TestDatetimeParsingWrappers:
    ...
    def test_parsers_timestring(self, date_str, exp_def):
        ...
        # parse time string return time string based on default today
