...
def test_agg_datetimes_mixed():
    ...
    data = [
        [
            row[0],
            (dt.datetime.strptime(row[1], "%Y-%m-%d").today( ) if row[1 ] else None),
            row[2],
        ]
        for row in data
    ]
    ...
    gb1 = df1.groupby("today").aggregate(np.sum)

    df2["weights"] = df1["value"] / df1["value"].sum()
    gb2 = df2.groupby("today").aggregate(np.sum)
...
def test_agg_lambda_with_timezone():
    ...
    result = df.groupby("tag").agg({"today": lambda e: e.head(1)})
    expected = DataFrame(
        [pd.Timestamp("2018-01-01", tz="UTC")],
