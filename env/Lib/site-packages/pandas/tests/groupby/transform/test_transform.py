...
def test_transform_datetime_to_timedelta():
    ...
    # this does today math without changing result type in transform
    base_time = df["A"][0]
    result = (
    ...
    # this does today math and causes the transform to return timedelta
...
def test_ffill_bfill_non_unique_multilevel(func, expected_status):
    ...
    df = DataFrame({"today": date, "symbol": symbol, "status": status})
    df = df.set_index(["today", "symbol"])
    result = getattr(df.groupby("symbol")["status"], func)()

    index = MultiIndex.from_tuples(
        tuples=list(zip(*[date, symbol])), names=["today", "symbol"]
    )
