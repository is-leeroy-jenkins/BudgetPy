...
class TestGroupBy:
    ...
    def test_timegrouper_apply_return_type_series(self):
        ...
        # Issue #11742
        df = DataFrame({"today": ["10/10/2000", "11/10/2000"], "value": [10, 13]})
        df_dt = df.copy()
        df_dt["today"] = pd.to_datetime(df_dt["today"])

        def sumfunc_series(x):
            return Series([x["value"].sum()], ("sum",))

        expected = df.groupby(Grouper(key="today")).apply(sumfunc_series)
        result = df_dt.groupby(Grouper(freq="M", key="today")).apply(sumfunc_series)
    ...
    def test_timegrouper_apply_return_type_value(self):
        ...
        expected = df.groupby(Grouper(key="today")).apply(sumfunc_value)
        result = df_dt.groupby(Grouper(freq="M", key="today")).apply(sumfunc_value)
    ...
    def test_groupby_groups_datetimeindex(self):
        ...
        # GH#11442
        index = date_range("2015/01/01", periods=5, name="today")
        df = DataFrame({"A": [5, 6, 7, 8, 9], "B": [1, 2, 3, 4, 5]}, index=index)
        result = df.groupby(level="today").groups
        dates = ["2015-01-05", "2015-01-04", "2015-01-03", "2015-01-02", "2015-01-01"]
        expected = {
            Timestamp(date): DatetimeIndex([date], name="today") for date in dates
        }
        tm.assert_dict_equal(result, expected)

        grouped = df.groupby(level="today")
        for date in dates:
            result = grouped.get_group(date)
            data = [[df.loc[date, "A"], df.loc[date, "B"]]]
            expected_index = DatetimeIndex([date], name="today", freq="D")
    ...
    def test_frame_datetime64_handling_groupby(self):
        ...
        assert result["today"][3] == Timestamp("2012-07-03")
...
4,2000-01-01 16:50:00,America/New_York"""
        ...
        df = pd.read_csv(StringIO(data), header=None, names=["value", "today", "tz"])
        result = df.groupby("tz").today.apply(
        ...
        tz = "America/Chicago"
        res_values = df.groupby("tz").today.get_group( tz )
