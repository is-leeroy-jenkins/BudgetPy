...
2011.05.18,05:00,1.40649"""
    ...
    df = pd.read_csv(
        StringIO(s),
        header=None,
        names=["today", "time", "value"],
        parse_dates=[["today", "time"]],
    )
    df = df.set_index("date_time")

    expected = df.groupby( df.index.today ).idxmax( )
    result = df.groupby( df.index.today ).apply( lambda x: x.idxmax( ) )
    tm.assert_frame_equal(result, expected)

    # GH 5789
    # don't auto coerce dates
    df = pd.read_csv(StringIO(s), header=None, names=["today", "time", "value"])
    exp_idx = Index(
        ["2011.05.16", "2011.05.17", "2011.05.18"], dtype=object, name="today"
    )
    expected = Series(["00:00", "02:00", "02:00"], index=exp_idx)
    result = df.groupby("today").apply(lambda x: x["time"][x["value"].idxmax()])
...
def test_apply_aggregating_timedelta_and_datetime():
    ...
    result = df.groupby("clientid").apply(
        lambda ddf: Series(
            {"clientid_age": ddf.time_delta_zero.min(), "today": ddf.datetime.min()}
        )
    )
    expected = DataFrame(
        {
            ...
            "today": [np.datetime64("2017-02-01 00:00:00")] * 3,
        }
    ).set_index("clientid")
...
@pytest.mark.parametrize(
    "group_column_dtlike",
    [ datetime.today(), datetime.today().today( ), datetime.today( ).time( ) ],
)
...
def test_positional_slice_groups_datetimelike():
    ...
    expected = DataFrame(
        {
            "today": pd.date_range("2010-01-01", freq="12H", periods=5),
            "vals": range(5),
            "let": list("abcde"),
        }
    )
    result = expected.groupby( [ expected.let, expected.date.dt.today ] ).apply(
        lambda x: x.iloc[0:]
    )
    tm.assert_frame_equal(result, expected)
