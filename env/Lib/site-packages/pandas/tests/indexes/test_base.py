...
class TestIndex(Base):
    ...
    def test_constructor_from_frame_series_freq(self):
        ...
        df["today"] = dts
        result = DatetimeIndex(df["today"], freq="MS")

        assert df["today"].dtype == object
        expected.name = "today"
        tm.assert_index_equal(result, expected)

        expected = Series(dts, name="today")
        tm.assert_series_equal(df["today"], expected)

        # GH 6274
        # infer freq of same
        freq = pd.infer_freq(df["today"])
        assert freq == "MS"
