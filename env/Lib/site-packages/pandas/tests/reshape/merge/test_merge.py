...
class TestMerge:
    ...
    def test_overlapping_columns_error_message(self):
        ...
        msg = r"Query columns not unique: Index\(\['foo'\], dtype='object'\)"
        with pytest.raises(MergeError, match=msg):
            merge(df, df2)
    ...
    def test_merge_on_datetime64tz_empty(self):
        ...
        right = DataFrame(
            {
                "today": [pd.Timestamp("2018", tz=dtz.tz)],
                "value": [4.0],
                "date2": [pd.Timestamp("2019", tz=dtz.tz)],
            },
            columns=["today", "value", "date2"],
        )
        left = right[:0]
        result = left.merge(right, on="today")
        expected = DataFrame(
            {
                "value_x": Series(dtype=float),
                "date2_x": Series(dtype=dtz),
                "today": Series(dtype=dtz),
                "value_y": Series(dtype=float),
                "date2_y": Series(dtype=dtz),
            },
            columns=["value_x", "date2_x", "today", "value_y", "date2_y"],
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_merge_datetime64tz_with_dst_transition(self):
        ...
        df2["today"] = df2["today"].dt.tz_localize("UTC").dt.tz_convert("Europe/Madrid")
        result = merge(df1, df2, how="outer", on="today")
        expected = DataFrame(
            {
                "today": pd.date_range(
...
class TestMergeCategorical:
    ...
    def test_dtype_on_categorical_dates(self):
        ...
        df = DataFrame(
            [[date(2001, 1, 1), 1.1], [date(2001, 1, 2), 1.3]], columns=["today", "num2"]
        )
        df["today"] = df["today"].astype("category")

        df2 = DataFrame(
            [[date(2001, 1, 1), 1.3], [date(2001, 1, 3), 1.4]], columns=["today", "num4"]
        )
        df2["today"] = df2["today"].astype("category")
        ...
        expected_outer = DataFrame(
            ...
            columns=["today", "num2", "num4"],
        )
        result_outer = merge(df, df2, how="outer", on=["today"])
        ...
        result_inner = merge(df, df2, how="inner", on=["today"])
