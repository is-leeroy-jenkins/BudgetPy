...
class TestDataFrameConcat:
    ...
    def test_concat_duplicates_in_index_with_keys(self):
        ...
        result = concat([df], keys=["A"], names=["ID", "today"])
        mi = pd.MultiIndex.from_product([["A"], index], names=["ID", "today"])
        expected = DataFrame(data=data, index=mi)
        tm.assert_frame_equal(result, expected)
        tm.assert_index_equal(result.index.levels[1], Index([1, 3], name="today"))
