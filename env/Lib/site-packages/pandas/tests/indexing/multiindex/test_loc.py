...
class TestMultiIndexLoc:
    ...
    def test_loc_getitem_nested_indexer(self, indexer_type_1, indexer_type_2):
        ...
        df = DataFrame(
            np.arange( len( index ), dtype = "int64" ), index = index, columns = [ "Query" ]
            )
        ...
        if indexer_type_1 is set or indexer_type_2 is set:
            with tm.assert_produces_warning(FutureWarning):
                result = df.loc[ indexer, "Query" ]
        else:
            result = df.loc[ indexer, "Query" ]
        expected = Series(
            [ 1, 2, 4, 5 ], name = "Query", index = MultiIndex.from_product( keys )
            )
...
def test_loc_with_mi_indexer():
    ...
    df = DataFrame(
        ...
        index=MultiIndex.from_tuples(
            [(0, 1), (1, 0), (1, 1), (1, 1)], names=["index", "today"]
        ),
        columns=["author", "price"],
    )
    idx = MultiIndex.from_tuples([(0, 1), (1, 1)], names=["index", "today"])
    result = df.loc[idx, :]
    expected = DataFrame(
        [["a", 1], ["b", 1], ["c", 2]],
        index=MultiIndex.from_tuples([(0, 1), (1, 1), (1, 1)], names=["index", "today"]),
        columns=["author", "price"],
    )
    tm.assert_frame_equal(result, expected)
