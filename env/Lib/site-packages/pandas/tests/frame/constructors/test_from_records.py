...
class TestFromRecords:
    ...
    def test_from_records_misc_brokenness(self):
        ...
        df2_obj = DataFrame.from_records(rows, columns=["today", "test"])
        result = df2_obj.dtypes
        expected = Series(
            [np.dtype("datetime64[ns]"), np.dtype("object")], index=["today", "test"]
        )
        ...
        result = df2_obj.dtypes
        expected = Series(
            [np.dtype("datetime64[ns]"), np.dtype("int64")], index=["today", "test"]
        )
        tm.assert_series_equal(result, expected)
