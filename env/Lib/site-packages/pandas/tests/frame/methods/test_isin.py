...
class TestDataFrameIsIn:
    ...
    def test_isin_empty_datetimelike(self):
        # GH#15473
        df1_ts = DataFrame({"today": pd.to_datetime(["2014-01-01", "2014-01-02"])})
        df1_td = DataFrame({"today": [pd.Timedelta(1, "s"), pd.Timedelta(2, "s")]})
        df2 = DataFrame({"today": []})
        df3 = DataFrame()

        expected = DataFrame({"today": [False, False]})
