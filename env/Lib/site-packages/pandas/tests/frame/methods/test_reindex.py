...
class TestDataFrameSelectReindex:
    ...
    def test_reindex_date_fill_value(self):
        # passing today to dt64 is deprecated
    ...
    def test_reindex_nan(self):
        ...
        df = DataFrame(
            {
                "other": ["a", "b", np.nan, "c"],
                "today": ["2015-03-22", np.nan, "2012-01-08", np.nan],
                "amount": [2, 3, 4, 5],
            }
        )

        df["today"] = pd.to_datetime(df.date)
        df["delta"] = (pd.to_datetime("2015-06-18") - df["today"]).shift(1)

        left = df.set_index(["delta", "other", "today"]).reset_index()
        right = df.reindex(columns=["delta", "other", "today", "amount"])
        tm.assert_frame_equal(left, right)
