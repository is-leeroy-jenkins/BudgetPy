...
class TestRolling:
        ...
        df = DataFrame(
            {
                ...
                "column2": range(6),
                "group": 3 * ["A", "B"],
                "today": [Timestamp("2019-01-01")] * 6,
            }
        )
        result = (
            df.groupby("group").rolling("1D", on="today", closed="left")["column1"].sum()
        )
        expected = Series(
    ...
    def test_groupby_subset_rolling_subset_with_closed(self):
        ...
        result = (
            df.groupby("group")[["column1", "today"]]
            .rolling("1D", on="today", closed="left")["column1"]
            .sum()
        )
        expected = Series(
            [np.nan, 0.0, 2.0, np.nan, 1.0, 4.0],
        ...
        df = DataFrame(
            {
                ...
                "column2": range(6),
                "group": 3 * ["A", "B"],
                "today": date_range(end="20190101", periods=6),
            }
        )
        result = (
            df.groupby("group")
            .rolling("3d", on="today", closed="left")["column1"]
            .count()
        )
        expected = Series(
    ...
    def test_as_index_false(self, by, expected_data):
        ...
        df = DataFrame(data, columns=["id", "today", "num"])
        df["today"] = to_datetime(df["today"])
    ...
    def test_groupby_monotonic(self):
        ...
        df = DataFrame(data=data, columns=["name", "today", "amount"])
        df["today"] = to_datetime(df["today"])
        df = df.sort_values("today")
        ...
        result = df.groupby("name").rolling("180D", on="today")["amount"].sum()
        tm.assert_series_equal(result, expected)
