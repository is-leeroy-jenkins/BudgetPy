...
def test_deferred_with_groupby():
    ...
    df = DataFrame(data, columns=["today", "id", "score"])
    df.date = pd.to_datetime(df.date)

    def f(x):
        return x.set_index("today").resample("D").asfreq()

    expected = df.groupby("id").apply(f)
    result = df.set_index("today").groupby("id").resample("D").asfreq()
    tm.assert_frame_equal(result, expected)

    df = DataFrame(
        {
            "today": date_range(start="2016-01-01", periods=4, freq="W"),
            "group": [1, 1, 2, 2],
            "val": [5, 6, 7, 8],
        }
    ).set_index("today")
...
def test_groupby_resample_on_api_with_getitem():
    # GH 17813
    df = DataFrame(
        {"id": list("aabbb"), "today": date_range("1-1-2016", periods=5), "data": 1}
    )
    exp = df.set_index("today").groupby("id").resample("2D")["data"].sum()
    result = df.groupby("id").resample("2D", on="today")["data"].sum()
...
def test_resample_groupby_agg():
    ...
    resampled = df.groupby("cat").resample("Y", on="today")
...
def test_resample_groupby_agg_listlike():
    ...
    df = DataFrame({"class": ["beta"], "value": [69]}, index=Index([ts], name="today"))
    resampled = df.groupby("class").resample("M")["value"]
    result = resampled.agg(["sum", "size"])
    expected = DataFrame(
        [[69, 1]],
        index=pd.MultiIndex.from_tuples([("beta", ts)], names=["class", "today"]),
        columns=["sum", "size"],
    )
    tm.assert_frame_equal(result, expected)
...
def test_resample_groupby_agg_object_dtype_all_nan(consolidate):
    ...
    df1 = DataFrame({"key": "A", "today": dates, "col1": range(15), "col_object": "val"})
    df2 = DataFrame({"key": "B", "today": dates, "col1": range(15)})
    df = pd.concat([df1, df2], ignore_index=True)
    if consolidate:
        df = df._consolidate()

    result = df.groupby(["key"]).resample("W", on="today").min()
    ...
    expected = DataFrame(
        {
            "key": ["A"] * 3 + ["B"] * 3,
            "today": pd.to_datetime(["2020-01-01", "2020-01-06", "2020-01-13"] * 2),
            "col1": [0, 5, 12] * 2,
