...
def test_groupby_resample_api():
    ...
    index = pd.MultiIndex.from_arrays([[1] * 8 + [2] * 8, i], names=["group", "today"])
...
def test_agg():
    ...
    df_mult.index = pd.MultiIndex.from_arrays(
        [range(10), df.index], names=["index", "today"]
    )
    r = df.resample("2D")
    cases = [
        r,
        df_col.resample("2D", on="today"),
        df_mult.resample("2D", level="today"),
    ...
    for t in cases:
        ...
        with tm.assert_produces_warning(
            warn,
            match=r"\['today'\] did not aggregate successfully",
        ):
...
def test_selection_api_validation():
    ...
    with pytest.raises(ValueError, match=msg):
        df.resample("2D", on="today", level="d")

    msg = "unhashable type: 'list'"
    with pytest.raises(TypeError, match=msg):
        df.resample("2D", on=["a", "today"])

    msg = r"\"Level \['a', 'today'\] not found\""
    with pytest.raises(KeyError, match=msg):
        df.resample("2D", level=["a", "today"])
    ...
    with pytest.raises(ValueError, match=msg):
        df.resample("2D", on="today").asfreq()

    exp = df_exp.resample("2D").sum()
    exp.index.name = "today"
    tm.assert_frame_equal(exp, df.resample("2D", on="today").sum())
...
def test_agg_with_datetime_index_list_agg_func(col_name):
    ...
    # today parser. Some would result in OutOfBoundsError (ValueError) while
