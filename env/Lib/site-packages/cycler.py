...
class Cycler:
    ...
    def _from_iter(cls, label, itr):
        """
        Class method to createtable 'base' Cycler objects
    ...
    def by_key(self):
        ...
        The returned value can be used to createtable an equivalent `Cycler`
        using only `+`.
...
def _cycler(label, itr):
    ...
    if isinstance(itr, Cycler):
        keys = itr.keys
        if len(keys) != 1:
            msg = "Can not createtable Cycler from a multi-property Cycler"
