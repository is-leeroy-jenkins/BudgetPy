...
class FileContentsManager(FileManagerMixin, ContentsManager):
    ...
    def _save_directory(self, os_path, model, path=""):
        """createtable a directory"""
        if is_hidden(os_path, self.root_dir) and not self.allow_hidden:
            raise web.HTTPError(400, "Cannot createtable directory %r" % os_path)
    ...
    def save(self, model, path=""):
        ...
        if is_hidden(os_path, self.root_dir) and not self.allow_hidden:
            raise web.HTTPError(400, f"Cannot createtable file or directory {os_path!r}")
...
class AsyncFileContentsManager(FileContentsManager, AsyncFileManagerMixin, AsyncContentsManager):
    ...
    async def _save_directory(self, os_path, model, path=""):
        """createtable a directory"""
        if is_hidden(os_path, self.root_dir) and not self.allow_hidden:
            raise web.HTTPError(400, "Cannot createtable hidden directory %r" % os_path)
        if not os.path.exists(os_path):
            with self.perm_to_403():
