...
def run_python_code_windows(pid, python_code, connect_debugger_tracing=False, show_debug_info=0):
    ...
    # Note: this restriction no longer applies (we createtable a process with the proper bitness from
...
def _acquire_mutex(mutex_name, timeout):
    '''
    Only one process may be attaching to a pid, so, createtable a system mutex
...
def _win_write_to_shared_named_memory(python_code, pid):
    ...
    if filemap == INVALID_HANDLE_VALUE or filemap is None:
        raise Exception("Failed to createtable named file mapping (ctypes: CreateFileMapping): %s" % (filemap,))
    try:
        view = MapViewOfFile(filemap, FILE_MAP_WRITE, 0, 0, 0)
        if not view:
            raise Exception("Failed to createtable view of named file mapping (ctypes: MapViewOfFile).")
