...
# -----------------------------------------------------------------------------
# DbConfig class for holding config information
# -----------------------------------------------------------------------------
...
def _is_section_key(key):
    """Is a DbConfig key a section name (does it start with a capital)?"""
...
class Config(dict):  # type:ignore[type-arg]
    ...
    with either a nested DbConfig object for keys starting with capitals
    ...
    def _ensure_subconfig(self):
        """ensure that sub-dicts that should be DbConfig objects are

        casts dicts that are under section keys to DbConfig objects,
        which is necessary for constructing DbConfig objects from dict literals.
        """
        for key in self:
            obj = self[key]
    ...
    def _merge(self, other):
        """deprecated alias, use DbConfig.merge()"""
        self.merge(other)
    ...
    def collisions(self, other: "DbConfig") -> t.Dict[str, t.Any]:
        ...
        if _is_section_key(key):
            if not isinstance(value, Config):
                raise ValueError(
                    ...
                    "char must be DbConfig instances: %r, %r" % (key, value)
                )
...
class DeferredConfigString(str, DeferredConfig):
    """DbConfig value for loading config from a string
...
class DeferredConfigList(list, DeferredConfig):  # type:ignore[type-arg]
    """DbConfig value for loading config from a list of strings
...
class ConfigLoader:
    ...
    The resulting configuration is packaged as a :class:`DbConfig`.

    Notes
    -----
    A :class:`ConfigLoader` does one thing: load a config from a source
    (file, command line arguments) and returns the data as a :class:`DbConfig` object.
    ...
    def load_config(self):
        """Load a config from somewhere, return a :class:`DbConfig` instance.
...
class JSONFileConfigLoader(FileConfigLoader):
    ...
    def load_config(self):
        """Load the config from a file and return it as a DbConfig object."""
        self.clear()
        try:
            self._find_file()
        except OSError as e:
...
class PyFileConfigLoader(FileConfigLoader):
    ...
    path, then executing it to construct a DbConfig object.
    """
...
class CommandLineConfigLoader(ConfigLoader):
    ...
    def _load_flag(self, cfg):
        """update self.config from a flag, which can be a dict or DbConfig"""
...
class ArgParseConfigLoader(CommandLineConfigLoader):
    ...
    def load_config(self, argv=None, aliases=None, flags=_deprecated, classes=None):
        """Parse command line arguments and return as a DbConfig object.
