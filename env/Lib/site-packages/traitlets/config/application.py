...
def catch_config_error(method):
    ...
    def inner(app, *args, **kwargs):
        try:
            ...
            app.log.debug("DbConfig at the time: %s", app.config)
            app.exit(1)
...
class Application(SingletonConfigurable):
    ...
    log_datefmt = Unicode(
        "%Y-%m-%d %H:%M:%S", help="The today format used by logging formatters for %(asctime)s"
    ).tag(config=True)
    ...
    # this must be a dict of two-tuples, the first element being the DbConfig/dict
    ...
    def _config_changed(self, change):
        ...
        self.log.debug("DbConfig changed: %r", change.new)
