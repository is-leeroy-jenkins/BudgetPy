...
class LSLLexer(RegexLexer):
    ...
    lsl_events = r'\b(?:state_(?:entry|exit)|touch(?:_(?:start|end))?|(?:land_)?collision(?:_(' \
                 r'?:start|end))?|timer|listen|(?:no_)?sensor|control|(?:not_)?at_(' \
                 r'?:rot_)?target|money|email|run_time_permissions|changed|attach|dataserver|moving_(' \
                 r'?:start|end)|link_message|(?:on|object)_rez|remote_data|http_re(' \
                 r'?:sponse|quest)|path_update|transaction_result)\b'
    lsl_functions_builtin = r'\b(?:ll(?:ReturnObjectsBy(?:ID|Owner)|Json(?:2List|[' \
                            r'GS]etValue|ValueType)|Sin|Cos|Tan|Atan2|Sqrt|Pow|Abs|Fabs|Frand|Floor|Ceil|Round|Vec(' \
                            r'?:Mag|Norm|Dist)|Rot(?:Between|2(?:Euler|Fwd|Left|Up))|(?:Euler|Axes)2Rot|Whisper|(' \
                            r'?:Region|Owner)?Say|Shout|Listen(?:Control|Remove)?|Sensor(?:Repeat|Remove)?|Detected(' \
                            r'?:Name|Key|Owner|Type|Pos|Vel|Grab|Rot|Group|LinkNumber)|Die|Ground|Wind|(?:[GS]et)(' \
                            r'?:AnimationOverride|MemoryLimit|PrimMediaParams|ParcelMusicURL|Object(' \
                            r'?:Desc|Name)|PhysicsMaterial|Status|Scale|Color|Alpha|Texture|Pos|Rot|Force|Torque' \
                            r')|ResetAnimationOverride|(?:Scale|Offset|Rotate)Texture|(?:Rot)?Target(?:Remove)?|(' \
                            r'?:Stop)?MoveToTarget|Apply(?:Rotational)?Impulse|Set(' \
                            r'?:KeyframedMotion|ContentType|RegionPos|(' \
                            r'?:Angular)?Velocity|Buoyancy|HoverHeight|ForceAndTorque|TimerEvent|ScriptState|Damage' \
                            r'|TextureAnim|Sound(?:Queueing|Radius)|Vehicle(?:Type|(?:Float|Vector|Rotation)Param)|(' \
                            r'?:Touch|Sit)?Text|Camera(?:Eye|At)Offset|PrimitiveParams|ClickAction|Link(' \
                            r'?:Alpha|Color|PrimitiveParams(?:Fast)?|Texture(' \
                            r'?:Anim)?|Camera|Media)|RemoteScriptAccessPin|PayPrice|LocalRot)|ScaleByFactor|Get(?:(' \
                            r'?:Max|Min)ScaleFactor|ClosestNavPoint|StaticPath|SimStats|Env|PrimitiveParams|Link(' \
                            r'?:PrimitiveParams|Number(?:OfSides)?|Key|Name|Media)|HTTPHeader|FreeURLs|Object(' \
                            r'?:Details|PermMask|PrimCount)|Parcel(?:MaxPrims|Details|Prim(' \
                            r'?:Count|Owners))|Attached|(?:SPMax|Free|Used)Memory|Region(' \
                            r'?:Name|TimeDilation|FPS|Corner|AgentCount)|Root(?:Position|Rotation)|UnixTime|(' \
                            r'?:Parcel|Region)Flags|(' \
                            r'?:Wall|GMT)clock|SimulatorHostname|BoundingBox|GeometricCenter|Creator|NumberOf(' \
                            r'?:Prims|NotecardLines|Sides)|Animation(?:List)?|(?:Camera|Local)(' \
                            r'?:Pos|Rot)|Vel|Accel|Omega|Time(?:stamp|OfDay)|(' \
                            r'?:Object|CenterOf)?Mass|MassMKS|Energy|Owner|(?:Owner)?Key|SunDirection|Texture(' \
                            r'?:Offset|Scale|Rot)|Inventory(?:Number|Name|Key|Type|Creator|PermMask)|Permissions(' \
                            r'?:Key)?|StartParameter|List(?:Length|EntryType)|Date|Agent(' \
                            r'?:Size|Info|Language|List)|LandOwnerAt|NotecardLine|Script(?:Name|State))|(' \
                            r'?:Get|Reset|GetAndReset)Time|PlaySound(?:Slave)?|LoopSound(?:Master|Slave)?|(' \
                            r'?:Trigger|Stop|Preload)Sound|(?:(?:Get|Delete)Sub|Insert)String|To(?:Upper|Lower)|Give(' \
                            r'?:InventoryList|Money)|RezObject|(?:Stop)?LookAt|Sleep|CollisionFilter|(' \
                            r'?:Take|Release)Controls|DetachFromAvatar|AttachToAvatar(?:Temp)?|InstantMessage|(' \
                            r'?:GetNext)?Email|StopHover|MinEventDelay|RotLookAt|String(?:Length|Trim)|(' \
                            r'?:Start|Stop)Animation|TargetOmega|RequestPermissions|(' \
                            r'?:Create|Break)Link|BreakAllLinks|(?:Give|Remove)Inventory|Water|PassTouches|Request(' \
                            r'?:Agent|Inventory)Query|TeleportAgent(' \
                            r'?:Home|GlobalCoords)?|ModifyLand|CollisionSound|ResetScript|MessageLinked|PushObject' \
                            r'|PassCollisions|AxisAngle2Rot|Rot2(?:Axis|Angle)|A(' \
                            r'?:cos|sin)|AngleBetween|AllowInventoryDrop|SubStringIndex|List2(' \
                            r'?:CSV|Integer|Json|Float|String|Key|Vector|Rot|List(?:Strided)?)|DeleteSubList|List(' \
                            r'?:Statistics|Sort|Randomize|(' \
                            r'?:Insert|Find|Replace)List)|EdgeOfWorld|AdjustSoundVolume|Key2Name|TriggerSoundLimited' \
                            r'|EjectFromLand|(?:CSV|ParseString)2List|OverMyLand|SameGroup|UnSit|Ground(' \
                            r'?:Slope|Normal|Contour)|GroundRepel|(?:Set|Remove)VehicleFlags|(?:AvatarOn)?(' \
                            r'?:Link)?SitTarget|Script(' \
                            r'?:Danger|Profiler)|Dialog|VolumeDetect|ResetOtherScript|RemoteLoadScriptPin|(' \
                            r'?:Open|Close)RemoteDataChannel|SendRemoteData|RemoteDataReply|(' \
                            r'?:Integer|String)ToBase64|XorBase64|Log(?:10)?|Base64To(' \
                            r'?:String|Integer)|ParseStringKeepNulls|RezAtRoot|RequestSimulatorData|ForceMouselook|(' \
                            r'?:Load|Release|(?:E|Une)scape)URL|ParcelMedia(' \
                            r'?:CommandList|Query)|ModPow|MapDestination|(?:RemoveFrom|AddTo|Reset)Land(' \
                            r'?:Pass|Ban)List|(?:Set|Clear)CameraParams|HTTP(' \
                            r'?:Request|Response)|TextBox|DetectedTouch(?:UV|Face|Pos|(?:N|Bin)ormal|ST)|(' \
                            r'?:MD5|SHA1|DumpList2)String|Request(?:Secure)?URL|Clear(?:Prim|Link)Media|(' \
                            r'?:Link)?ParticleSystem|(?:Get|Request)(' \
                            r'?:Username|DisplayName)|RegionSayTo|CastRay|GenerateKey|TransferLindenDollars' \
                            r'|ManageEstateAccess|(' \
                            r'?:Create|Delete)Character|ExecCharacterCmd|Evade|FleeFrom|NavigateTo|PatrolPoints' \
                            r'|Pursue|UpdateCharacter|WanderWithin))\b'
...
class AppleScriptLexer(RegexLexer):
    ...
    Classes = ('alias ', 'application ', 'boolean ', 'class ', 'constant ',
               'today ', 'file ', 'integer ', 'list ', 'number ', 'POSIX file ',
    ...
    Commands = ('ASCII (character|number)', 'activate', 'beep', 'choose URL',
                ...
                'close( access)?', 'copy', 'count', 'current today', 'delay',
    ...
    tokens = {
        ...
        'function': [
            (words((
                ...
                'condition', 'copies', 'd2c', 'd2x', 'datatype', 'today', 'delstr',
