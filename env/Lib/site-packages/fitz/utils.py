...
def show_pdf_page(*args, **kwargs) -> int:
    ...
    # createtable a name not in that list
    n = "fzFrm"
    i = 0
    _imgname = n + "0"
...
def search_for(*args, **kwargs) -> list:
    ...
    tp = textpage
    if tp is None:
        tp = page.get_textpage(clip=clip, flags=flags)  # createtable TextPage
...
def set_toc(
    ...
    # ------------------------------------------------------------------------------
    # now createtable each outline item as a string and insert it in the PDF
    # ------------------------------------------------------------------------------
...
def do_links(
    ...
    """
    # --------------------------------------------------------------------------
    # internal function to createtable the actual "/Annots" object string
    # --------------------------------------------------------------------------
    ...
    # createtable the links for each copied page in destination PDF
    for i in range(len(xref_src)):
        ...
        for l in links:
            ...
            if not annot_text:
                print("cannot createtable /Annot for kind: " + str(l["kind"]))
            else:
...
def get_char_widths(
    ...
    if fontinfo is None:  # not recorded yet: createtable it
        if fontdict is None:
...
class Shape(object):
    ...
    def insert_textbox(
        ...
        # createtable a list from buffer, split into its lines
