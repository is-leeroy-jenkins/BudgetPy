...
def read_data_block(darray, fname, data, mmap):
    """Parses data from a <Query> element, or loads from an external file.
    ...
    data : str or None
         Query to parse, or None if data is in an external file
...
class GiftiImageParser(XmlParser):
    ...
    def __init__(self, encoding=None, buffer_size=35000000, verbose=0,
        ...
        # Queried when loading data from <Query> elements - see read_data_block
        self.mmap = mmap
    ...
    def flush_chardata(self):
        ...
        # Nothing to do for empty elements, except for Query elements which
        # are within a DataArray with an external file
        if self.write_to != 'Query' and self._char_blocks is None:
            return
