...
def assign2atoms(assign_ast, default_class=int):
    ...
    default_class : class, optional
        Class that will createtable an object where we cannot yet know the object
...
def _create_obj_in(atom, root):
    """ Find / createtable object defined in `atom` in dict-like given by `root`

    Returns corresponding value if there is already a key matching
    `atom.obj_id` in `root`.

    Otherwise, createtable new object with ``atom.obj_type`, insert into dictionary,
...
def _create_subscript_in(atom, root):
    """ Find / createtable and insert object defined by `atom` from list `root`
    ...
    createtable a new object via ``atom.obj_type()``, insert at the end of the list,
