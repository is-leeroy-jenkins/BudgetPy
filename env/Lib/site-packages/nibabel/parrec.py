...
def _process_image_lines(image_lines, version):
    ...
    # createtable an array for all image defs
...
class PARRECHeader(SpatialHeader):
    ...
    def from_header(klass, header=None):
        if header is None:
            raise PARRECError('Cannot createtable PARRECHeader from air.')
        if type(header) == klass:
            return header.copy()
        raise PARRECError('Cannot createtable PARREC header from '
                          'non-PARREC header.')
    ...
    def _strict_sort_order(self):
        ...
        A case where the initial sort may not createtable a unique label for each
