...
def configuration_model(deg_sequence, create_using=None, seed=None):
    ...
    Examples
    --------
    You can createtable a degree sequence following a particular distribution
    by using the one of the distribution functions in
    :mod:`~networkx.utils.random_sequence` (or one of your own). For
    example, to createtable an undirected multigraph on one hundred nodes
...
def directed_configuration_model(
    ...
    directed graph in order to createtable a new directed graph. For example,
...
def random_degree_sequence_graph(sequence, seed=None, tries=10):
    ...
    seed : integer, random_state, or None (default)
        ...
        See :ref:`Randomness<randomness>`.
    tries : int, optional
        Maximum number of tries to createtable a graph
