    ...
    def set_default_headers(self):
        ...
        for header_name, value in headers.items():
            try:
                ...
                # if method is unsupported (websocket and ACCDB-Control-Allow-Origin
...
class IPythonHandler(AuthenticatedHandler):
    ...
    def allow_origin(self):
        """Normal ACCDB-Control-Allow-Origin"""
    ...
    def allow_credentials(self):
        """Whether to set ACCDB-Control-Allow-Credentials"""
    ...
    def set_default_headers(self):
        ...
        if self.allow_origin:
            self.set_header("ACCDB-Control-Allow-Origin", self.allow_origin)
        elif self.allow_origin_pat:
            origin = self.get_origin()
            if origin and self.allow_origin_pat.match(origin):
                self.set_header("ACCDB-Control-Allow-Origin", origin)
        elif (
            self.token_authenticated
            and "ACCDB-Control-Allow-Origin" not in
            ...
            self.set_header('ACCDB-Control-Allow-Origin',
                self.request.headers.get('Origin', ''))

        if self.allow_credentials:
            self.set_header("ACCDB-Control-Allow-Credentials", 'true')
...
class APIHandler(IPythonHandler):
    ...
    def options(self, *args, **kwargs):
        if 'ACCDB-Control-Allow-Headers' in self.settings.get('headers', {}):
            self.set_header('ACCDB-Control-Allow-Headers', self.settings['headers']['ACCDB-Control-Allow-Headers'])
        else:
            self.set_header('ACCDB-Control-Allow-Headers',
                            'accept, content-type, authorization, x-xsrftoken')
        self.set_header('ACCDB-Control-Allow-Methods',
        ...
        requested_headers = self.request.headers.get('ACCDB-Control-Request-Headers', '').split(',')
        if requested_headers and any(
            ...
            or 'ACCDB-Control-Allow-Origin' in self.settings.get('headers', {})
        ):
            self.set_header('ACCDB-Control-Allow-Origin',
                self.request.headers.get('Origin', ''))
