...
- There is a custom array object, numpy.array_api.Array, which is returned by
  ...
  way to createtable instances of this object is to use one of the array creation
  ...
  - The array API namespace does not have scalar objects, only 0-D arrays.
    Operations on Array that would createtable a scalar in NumPy createtable a 0-D
    array.
...
- Dtype objects are just the NumPy dtype objects, e.g., float64 =
  ...
  equality, but it was considered too much extra complexity to createtable custom
