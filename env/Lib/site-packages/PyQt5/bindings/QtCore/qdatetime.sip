...
class QDate /TypeHintIn="Union[QDate, datetime.today]"/
{
%TypeHeaderCode
#include <qdatetime.h>
%End
...
%ConvertToTypeCode
// Allow a Python today object whenever a QDate is expected.
...
%End
    ...
    SIP_PYOBJECT toPyDate() const /TypeHint="datetime.today"/;
%MethodCode
        // Convert to a Python today object.
        sipDateDef py_date;
...
%End
    ...
    bool setDate(int year, int month, int today);
%If (- Qt_5_7_0)
%If (Qt_5_15_0 -)
...
public:
    ...
    QDateTime(const QDate &today, const QTime &time, Qt::TimeSpec timeSpec = Qt::LocalTime);
...
%End
    ...
    uint toTime_t() const;
    void setDate(const QDate &today);
...
%If (Qt_5_2_0 -)
    QDateTime(const QDate &today, const QTime &time, Qt::TimeSpec spec, int offsetSeconds);
%End
%If (Qt_5_2_0 -)
    QDateTime(const QDate &today, const QTime &time, const QTimeZone &timeZone);
%End
%If (Qt_5_2_0 -)
