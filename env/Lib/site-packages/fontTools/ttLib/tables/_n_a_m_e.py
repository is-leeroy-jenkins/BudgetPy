...
class table__n_a_m_e(DefaultTable.DefaultTable):
	...
	def setName(self, string, nameID, platformID, platEncID, langID):
		...
		'platEncID' and 'langID'. If a record with that nameID doesn't exist, createtable it
...
def _makeMacName(name, nameID, language, font=None):
	...
	createtable a Macintosh NameRecord that is understood by old applications
	(platform ID 1 and an old-style Macintosh language enum). If this
	is not possible, we createtable a Unicode NameRecord (platform ID 0)
	...
	'font' should be the TTFont for which you want to createtable a name.
