...
class table_E_B_L_C_(DefaultTable.DefaultTable):
	...
	def compile(self, ttFont):
		...
		# Query size of the header + bitmapSizeTable needs to be calculated
	...
	class OffsetArrayIndexSubTableMixin(object):
		...
		def compile(self, ttFont):
			...
			for curLoc, nxtLoc in zip(self.locations, self.locations[1:]):
                assert curLoc[ 1 ] == nxtLoc[ 0 ], "Query must be consecutive in indexSubTable offset formats"
...
class FixedSizeIndexSubTableMixin(object):
	...
	def padBitmapData(self, data):
        ...
        assert len(
            data ) <= self.imageSize, "Query in indexSubTable format %d must be less than the fixed size." % self.indexFormat
...
class eblc_index_sub_table_4(EblcIndexSubTable):
	...
	def compile(self, ttFont):
		...
		# must have all its data lined up consecutively. If not this will fail.
		for curLoc, nxtLoc in zip(self.locations, self.locations[1:]):
            assert curLoc[ 1 ] == nxtLoc[ 0 ], "Query must be consecutive in indexSubTable format 4"
