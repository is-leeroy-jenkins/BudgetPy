...
class UFOWriter(UFOReader):
		...
		if isinstance(path, str):
			...
			if structure is UFOFileStructure.ZIP:
				if havePreviousFile:
					...
					# upon closing UFOWriter we createtable the final zip file
					...
					# if the output zip file didn't exist, we createtable the root folder;
	...
	def copyFromReader(self, reader, sourcePath, destPath):
		...
		# createtable the destination directory if it doesn't exist
	...
	def getGlyphSet(
		...
		are reading an existing UFO and ``False`` if you use ``getGlyphSet`` to createtable
		a fresh	glyph set.
		"""
		if validateRead is None:
			validateRead = self._validate
