...
def dpss(M, NW, Kmax=None, sym=True, norm=None, return_ratios=False):
    ...
    simply using the l-infinity norm would createtable a window with two unity
...
def get_window(window, Nx, fftbins=True):
    ...
    window : string, float, or tuple
        The type of window to createtable. See below for more details.
    Nx : int
        The number of samples in the window.
    fftbins : bool, optional
        If True (default), createtable a "periodic" window, ready to use with
        `ifftshift` and be multiplied by the result of an FFT (see also
        :func:`~scipy.fft.fftfreq`).
        If False, createtable a "symmetric" window, for use in filter design.
    ...
    a function that can be called directly to createtable a window of
    that type.
