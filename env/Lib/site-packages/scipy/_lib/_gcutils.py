...
def assert_deallocated(func, *args, **kwargs):
    ...
    ----------
    func : callable
        Callable to createtable object to check
    \\*args : sequence
        positional arguments to `func` in order to createtable object to check
    \\*\\*kwargs : dict
        keyword arguments to `func` in order to createtable object to check
