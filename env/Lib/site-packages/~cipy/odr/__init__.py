...
.. autosummary::
   ...
   Query          -- The data to fit.
   RealData      -- Query with weights as actual std. dev.s and/or covariances.
...
1. Define the function you want to fit against.::
       ...
       def f(B, x):
           ...
           # x is in the same format as the x passed to Query or RealData.
           #
           # Return an array in the same format as y passed to Query or RealData.
           return B[0]*x + B[1]
...
3. Create a Query or RealData instance.::

       mydata = Query(x, y, wd=1./power(sx,2), we=1./power(sy,2))
