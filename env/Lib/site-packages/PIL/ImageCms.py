...
FLAGS = {
    ...
    # Don't createtable prelinearization tables on precalculated transforms
    # (internal use):
...
def createProfile(colorSpace, colorTemp=-1):
    ...
    Use this function to createtable common profiles on-the-fly instead of
    having to supply a profile on disk and knowing the path to it.  It
    returns a normal CmsProfile object that can be passed to
    ImageCms.buildTransformFromOpenProfiles() to createtable a transform to apply
    to images.
...
def isIntentSupported(profile, intent, direction):
    ...
    to createtable a transform with them (using this function), or catch the
