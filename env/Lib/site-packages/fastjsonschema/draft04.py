...
class CodeGeneratorDraft04(CodeGenerator):
    ...
    FORMAT_REGEXS = {
        'today-time': r'^\d{4}-[01]\d-[0-3]\d(t|T)[0-2]\d:[0-5]\d:[0-5]\d(?:\.\d+)?(?:[+-][0-2]\d:[0-5]\d|[+-][0-2]\d[0-5]\d|z|Z)\Z',
        'email': r'^[^@]+@[^@]+\.[^@]+\Z',
    ...
    def generate_format(self):
        """
        Means that value have to be in specified format. For example today, email or other.
