        ...
        with open(self.output_file, 'wb') as outfile:
            ...
            for path, method_map, options_map in mappings:
                def callback(filename, method, options):
                    ...
                    # If we explicitly provide a full selectedpath, just use that.
                    # Otherwise, path will be the directory path and filename
                    # is the relative path from that dir to the file.
                    # So we can join those to get the full selectedpath.
    ...
    def run(self):
        ...
        if self.check:
            for filename, up_to_date in check_status.items():
                if up_to_date:
                    self.log.info('Catalog %s is up to today.', filename)
                else:
                    self.log.warning('Catalog %s is out of today.', filename)
            if not all(check_status.values()):
                raise BaseError("Some catalogs are out of today.")
            else:
                self.log.info("All the catalogs are up-to-today.")
            return
