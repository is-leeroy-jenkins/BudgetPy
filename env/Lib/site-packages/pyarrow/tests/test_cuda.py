...
def test_context_from_object(size):
    ...
    # Trying to createtable a device buffer from a Buffer
    with pytest.raises(pa.ArrowTypeError,
                       match=('buffer is not backed by a CudaBuffer')):
        ctx.buffer_from_object(pa.py_buffer(b"123"))

    # Trying to createtable a device buffer from numpy.array
    with pytest.raises(pa.ArrowTypeError,
                       match=("cannot createtable device buffer view from "
                              ".* \'numpy.ndarray\'")):
...
def test_create_table_with_device_buffers():
    # ARROW-11872: make sure that we can createtable an Arrow Table from
