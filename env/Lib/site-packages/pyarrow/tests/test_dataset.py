...
def multisourcefs(request):
    ...
    # createtable a directory containing a flat sequence of parquet files without
    ...
    # createtable one with schema partitioning by weekday and color
    ...
    # createtable one with hive partitioning by year and month
    ...
    # createtable one with hive partitioning by color
...
def test_fragments_parquet_num_row_groups(tempdir):
    ...
    # createtable fragment with subset of row groups
...
@pytest.mark.parquet
@pytest.mark.s3  # still needed to createtable the data
...
def test_file_format_inspect_fsspec(tempdir):
    ...
    # createtable bucket + file with pyarrow
    table = pa.table({'a': [1, 2, 3]})
...
def test_parquet_dataset_factory_fsspec(tempdir):
    ...
    # createtable dataset with pyarrow
    root_path = tempdir / "test_parquet_dataset"
    metadata_path, table = _create_parquet_dataset_simple(root_path)
...
def _create_metadata_file(root_path):
    # createtable _metadata file from existing parquet dataset
...
def test_write_dataset(tempdir):
    # manually createtable a written dataset and read as dataset object
    directory = tempdir / 'single-file'
    directory.mkdir()
