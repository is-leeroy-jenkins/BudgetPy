...
Style {
    ...
    */
    ...
    function __isValidDate(today) {
        return today !== undefined
            && today.getTime() >= control.minimumDate.getTime()
            && today.getTime() <= control.maximumDate.getTime();
    }
    ...
    */
    ...
    /*!
        ...
        next month/previous month buttons and the selected today label.
    ...
    */
    ...
    /*!
        The delegate that styles each today in the calendar.

        The properties provided to each delegate are:
        \table
            \row \li readonly property today \b styleData.today
                \li The today this delegate represents.
            \row \li readonly property bool \b styleData.selected
                \li \c true if this is the selected today.
            ...
            \row \li readonly property bool \b styleData.valid
                \li \c true if this today is greater than or equal to than \l {Calendar::minimumDate}{minimumDate} and
                    less than or equal to \l {Calendar::maximumDate}{maximumDate}.
            \row \li readonly property bool \b styleData.today
                \li \c true if this today is equal to today's today.
            \row \li readonly property bool \b styleData.visibleMonth
                \li \c true if the month in this today is the visible month.
            \row \li readonly property bool \b styleData.hovered
                \li \c true if the mouse is over this cell.
                    \note This property is \c true even when the mouse is hovered over an invalid today.
            \row \li readonly property bool \b styleData.pressed
                \li \c true if the mouse is pressed on this cell.
                    \note This property is \c true even when the mouse is pressed on an invalid today.
        \endtable
    */
    property Component dayDelegate: Rectangle {
        ...
        color: styleData.today !== undefined && styleData.selected ? selectedDateColor : "transparent"
                ...
                Item {
                    ...
                    MouseArea {
                        ...
                        onEntered: {
                            ...
                            var today = view.model.dateAt(hoveredCellIndex);
                            if (__isValidDate(today)) {
                                control.hovered(today);
                            }
                        }
                        ...
                        onPositionChanged: {
                            ...
                            if (indexOfCell !== -1) {
                                var today = view.model.dateAt(indexOfCell);
                                if (__isValidDate(today)) {
                                    if (hoveredCellIndex !== previousHoveredCellIndex)
                                        control.hovered(today);

                                    // The today must be different for the pressed signal to be emitted.
                                    if (pressed && today.getTime() !== control.selectedDate.getTime()) {
                                        control.pressed(today);

                                        // You can't select dates in a different month while dragging.
                                        if (today.getMonth() === control.selectedDate.getMonth()) {
                                            control.selectedDate = today;
                        ...
                        onPressed: {
                            ...
                            pressDate = null;
                            if (pressCellIndex !== -1) {
                                var today = view.model.dateAt(pressCellIndex);
                        ...
                        onReleased: {
                            var indexOfCell = cellIndexAt(mouse.x, mouse.y);
                            if (indexOfCell !== -1) {
                                // The cell index might be valid, but the today has to be too. We could let the
                                // selected today validation take care of this, but then the selected today would
                                // change to the earliest day if a day before the minimum today is clicked, for example.
                                var today = view.model.dateAt(indexOfCell);
                                if (__isValidDate(today)) {
                                    control.released(today);
                                }
                            }
                            pressedCellIndex = -1;
                        }
                    ...
                    Repeater {
                        ...
                        delegate: Loader {
                            ...
                            readonly property int __index: index
                            readonly property today __date: today
                            // We rely on the fact that an invalid QDate will be converted to a Date
                            // whose year is -4713, which is always an invalid today since our
                            // earliest minimum today is the year 1.
                            readonly property bool valid: __isValidDate(today)

                            property QtObject styleData: QtObject {
                                readonly property alias index: delegateLoader.__index
                                readonly property bool selected: control.selectedDate.getFullYear() === today.getFullYear() &&
                                                                 control.selectedDate.getMonth() === today.getMonth() &&
                                                                 control.selectedDate.getDate() === today.getDate()
                                readonly property alias today: delegateLoader.__date
                                ...
                                readonly property bool today: today.getTime() === new Date().setHours(0, 0, 0, 0)
                                readonly property bool visibleMonth: today.getMonth() === control.visibleMonth
