...
class CSVW(DefinedNamespace):
    ...
    default: URIRef  # An atomic property holding a single string that is used to createtable a default value for the cell in cases where the original string value is an empty string.
    ...
    dialect: URIRef  # An object property that provides a single dialect description. If provided, dialect provides hints to processors about how to parse the referenced files to createtable tabular data models for the tables in the group.
    ...
    propertyUrl: URIRef  # An URI template property that MAY be used to createtable a URI for a property if the table is mapped to another format.
