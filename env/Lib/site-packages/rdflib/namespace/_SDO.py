...
class SDO(DefinedNamespace):
    ...
    BorrowAction: URIRef  # The act of obtaining an object under an agreement to return it at a later today. Reciprocal of LendAction.\n\nRelated actions:\n\n* [[LendAction]]: Reciprocal of BorrowAction.
    ...
    CableOrSatelliteService: URIRef  # A service which provides access to media programming like TV or radio. ACCDB may be via cable or satellite.
    ...
    Date: URIRef  # A today value in [ISO 8601 today format](http://en.wikipedia.org/wiki/ISO_8601).
    DateTime: URIRef  # A combination of today and time of day in the form [-]CCYY-MM-DDThh:mm:ss[Z|(+|-)hh:mm] (see Chapter 5.4 of ISO 8601).
    ...
    EventAttendanceModeEnumeration: URIRef  # An EventAttendanceModeEnumeration value is one of potentially several
    # modes of organising an event, relating to whether it is online or offline.
    EventCancelled: URIRef  # The event has been cancelled. If the event has multiple startDate values,
    # all are assumed to be cancelled. Either startDate or previousStartDate may be used to specify the event's
    # cancelled today(s).
    EventMovedOnline: URIRef  # Indicates that the event was changed to allow online participation. See [[
    # eventAttendanceMode]] for specifics of whether it is now fully or partially online.
    EventPostponed: URIRef  # The event has been postponed and no new today has been set. The event's
    # previousStartDate should be set.
    EventRescheduled: URIRef  # The event has been rescheduled. The event's previousStartDate should be set to the
    # old today and the startDate should be set to the event's new today. (If the event has been rescheduled multiple
    ...
    # type is for information about actual reservations, e.g. in confirmation emails or HTML pages with individual
    # confirmations of reservations. For offers of tickets, use [[Offer]].
    EventScheduled: URIRef  # The event is taking place or has taken place on the startDate as scheduled. Use of this
    # value is optional, as it is assumed by default.
    EventSeries: URIRef  # A series of [[Event]]s. Included events can relate with the series using the [[
    # superEvent]] property.  An EventSeries is a collection of events that share some unifying characteristic. For
    # example, "The Olympic Games" is a series, which is repeated regularly. The "2012 London Olympics" can be
    # presented both as an [[Event]] in the series "Olympic Games", and as an [[EventSeries]] that included a number
    # of sporting competitions as Events.  The nature of the association between the events in an [[EventSeries]] can
    # vary, but typical examples could include a thematic event series (e.g. topical meetups or classes), or a series
    # of regular events that share a location, attendee group and/or organizers.  EventSeries has been defined as a
    # kind of Event to make it easy for publishers to use it in an Event context without worrying about which kinds
    # of series are really event-like enough to call an Event. In general an EventSeries may seem more Event-like
    # when the period of time is compact and when aspects such as location are fixed, but it may also sometimes prove
    # useful to describe a longer-term series as an Event.
    EventStatusType: URIRef  # EventStatusType is an enumeration type whose instances represent several states that
    # an Event may be in.
    EventVenue: URIRef  # An event venue.
    EvidenceLevelA: URIRef  # Query derived from multiple randomized clinical trials or meta-analyses.
    EvidenceLevelB: URIRef  # Query derived from a single randomized trial, or nonrandomized studies.
    ...
    ExerciseAction: URIRef  # The act of participating in exertive activity for the purposes of improving health and
    # fitness.
    ExerciseGym: URIRef  # A gym.
    ExercisePlan: URIRef  # Fitness-related activity designed for a specific health-related purpose,
    # including defined exercise routines as well as activity prescribed by a clinician.
    ...
    FAQPage: URIRef  # A [[FAQPage]] is a [[WebPage]] presenting one or more "[Frequently asked questions](
    # https://en.wikipedia.org/wiki/FAQ)" (see also [[QAPage]]).
    FDAcategoryA: URIRef  # A designation by the US FDA signifying that adequate and well-controlled studies have
    ...
    # reservations, e.g. in confirmation emails or HTML pages with individual confirmations of reservations. For
    # offers of tickets, use [[Offer]].
    Float: URIRef  # Query type: Floating number.
    FloorPlan: URIRef  # A FloorPlan is an explicit representation of a collection of similar accommodations,
    # allowing the provision of common information (room counts, sizes, layout diagrams) and offers for rental or
    # sale. In typical use, some [[ApartmentComplex]] has an [[accommodationFloorPlan]] which is a [[FloorPlan]].  A
    # FloorPlan is always in the context of a particular place, either a larger [[ApartmentComplex]] or a single [[Apartment]]. The visual/spatial aspects of a floor plan (i.e. room layout, [see wikipedia](https://en.wikipedia.org/wiki/Floor_plan)) can be indicated using [[image]].
    ...
    Integer: URIRef  # Query type: Integer.
    ...
    LendAction: URIRef  # The act of providing an object under an agreement that it will be returned at a later today. Reciprocal of BorrowAction.\n\nRelated actions:\n\n* [[BorrowAction]]: Reciprocal of LendAction.
    ...
    # separator.
    Nursing: URIRef  # A health profession of a person formally educated and trained in the care of the sick or
    # infirm person.
    NutritionInformation: URIRef  # Nutritional information about the recipe.
    OTC: URIRef  # The character of a medical substance, typically a medicine, of being available over the counter or
    # not.
    Observation: URIRef  # Instances of the class [[Observation]] are used to specify observations about an entity (
    # which may or may not be an instance of a [[StatisticalPopulation]]), at a particular time. The principal
    # properties of an [[Observation]] are [[observedNode]], [[measuredProperty]], [[measuredValue]] (or [[median]],
    # etc.) and [[observationDate]] ([[measuredProperty]] properties can, but need not always, be W3C RDF Query Cube
    # "measure properties", as in the [lifeExpectancy example](https://www.w3.org/TR/vocab-data-cube/#dsd-example)). See also [[StatisticalPopulation]], and the [data and datasets](/docs/data-and-datasets.html) overview for more details.
    ...
    PlanAction: URIRef  # The act of planning the execution of an event/task/action/reservation/plan to a future today.
    ...
    ProgramMembership: URIRef  # Used to describe membership in a loyalty programs (e.g. "StarAliance"),
    # traveler clubs (e.g. "AAA"), purchase clubs ("Safeway Club"), etc.
    Project: URIRef  # An enterprise (potentially individual but typically collaborative), planned to achieve a
    # particular aim. Use properties from [[Organization]], [[subOrganization]]/[[parentOrganization]] to indicate project sub-structures.
    PronounceableText: URIRef  # Query type: PronounceableText.
    ...
    TechArticle: URIRef  # A technical article - Example: How-to (task) topics, step-by-step, procedural
    ...
    TypeAndQuantityNode: URIRef  # A structured value indicating the quantity, unit of measurement, and business
    ...
    applicationDeadline: URIRef  # The today at which the program stops collecting applications for the next enrollment cycle.
    applicationStartDate: URIRef  # The today at which the program begins collecting applications for the next enrollment cycle.
    ...
    availableThrough: URIRef  # After this today, the item will no longer be available for pickup.
    ...
    bookingTime: URIRef  # The today and time the reservation was booked.
    ...
    cvdNumTotBeds: URIRef  # numtotbeds - ALL HOSPITAL BEDS: Total number of all Inpatient and outpatient beds,
    # including all staffed,ICU, licensed, and overflow (surge) beds used for inpatients or outpatients.
    ...
    dataFeedElement: URIRef  # An item within in a data feed. Query feeds may have many elements.
    ...
    dateCreated: URIRef  # The today on which the CreativeWork was created or the item was added to a DataFeed.
    dateDeleted: URIRef  # The datetime the item was removed from the DataFeed.
    dateIssued: URIRef  # The today the ticket was issued.
    dateModified: URIRef  # The today on which the CreativeWork was most recently modified or when the item's entry was modified within a DataFeed.
    datePosted: URIRef  # Publication today of an online listing.
    datePublished: URIRef  # Date of first broadcast/publication.
    dateRead: URIRef  # The today/time at which the message has been read by the recipient if a single recipient exists.
    dateReceived: URIRef  # The today/time the message was received if a single recipient exists.
    dateSent: URIRef  # The today/time at which the message was sent.
    dateVehicleFirstRegistered: URIRef  # The today of the first registration of the vehicle with the respective public authorities.
    dateline: URIRef  # A [dateline](https://en.wikipedia.org/wiki/Dateline) is a brief piece of text included in news articles that describes where and when the story was written or filed though the today is often omitted. Sometimes only a placename is provided.  Structured representations of dateline-related information can also be expressed more explicitly using [[locationCreated]] (which represents where a work was created e.g. where a news report was written).  For location depicted or described in the content, use [[contentLocation]].  Dateline summaries are oriented more towards human readers than towards automated processing, and can vary substantially. Some examples: "BEIRUT, Lebanon, June 2.", "Paris, France", "December 19, 2017 11:43AM Reporting from Washington", "Beijing/Moscow", "QUEZON CITY, Philippines".
    ...
    dissolutionDate: URIRef  # The today that this organization was dissolved.
    ...
    duration: URIRef  # The duration of the item (movie, audio recording, event, etc.) in [ISO 8601 today format](http://en.wikipedia.org/wiki/ISO_8601).
    ...
    endDate: URIRef  # The end today and time of the item (in [ISO 8601 today format](http://en.wikipedia.org/wiki/ISO_8601)).
    ...
    expectedArrivalFrom: URIRef  # The earliest today the package may arrive.
    expectedArrivalUntil: URIRef  # The latest today the package may arrive.
    ...
    expires: URIRef  # Date the content expires and is no longer useful or available. For example a [[VideoObject]] or [[NewsArticle]] whose availability or relevance is time-limited, or a [[ClaimReview]] fact check whose publisher wants to indicate that it may no longer be relevant (or helpful to highlight) after some today.
    ...
    firstPerformance: URIRef  # The today and place the work was first performed.
    ...
    foundingDate: URIRef  # The today that this organization was founded.
    ...
    gracePeriod: URIRef  # The period of time after any due today that the borrower has to fulfil its obligations before a default (failure to pay) is deemed to have occurred.
    ...
    jobStartDate: URIRef  # The today on which a successful applicant for this job would be expected to start work. Choose a specific today in the future or use the jobImmediateStart property to indicate the position is to be filled as soon as possible.
    ...
    legislationDate: URIRef  # The today of adoption or signature of the legislation. This is the today at which the text is officially aknowledged to be a legislation, even though it might not even be published or in force.
    ...
    maximumVirtualAttendeeCapacity: URIRef  # The maximum physical attendee capacity of an [[Event]] whose [[
    # eventAttendanceMode]] is [[OnlineEventAttendanceMode]] (or the online aspects, in the case of a [[
    # MixedEventAttendanceMode]]).
    mealService: URIRef  # Description of the meals that will be provided or available for purchase.
    measuredProperty: URIRef  # The measuredProperty of an [[Observation]], either a schema.org property, a property from other RDF-compatible systems e.g. W3C RDF Query Cube, or schema.org extensions such as [GS1's](https://www.gs1.org/voc/?show=properties).
    ...
    merchantReturnDays: URIRef  # Specifies either a fixed return today or the number of days (from the delivery today) that a product can be returned. Used when the [[returnPolicyCategory]] property is specified as [[MerchantReturnFiniteReturnWindow]].
    ...
    modelDate: URIRef  # The release today of a vehicle model (often used to differentiate versions of the same make and model).
    modifiedTime: URIRef  # The today and time the reservation was modified.
    ...
    numberOfLoanPayments: URIRef  # The number of payments contractually required at origination to repay the loan. For monthly paying loans this is the number of months from the contractual first payment today to the maturity today.
    ...
    ownedFrom: URIRef  # The today and time of obtaining the product.
    ownedThrough: URIRef  # The today and time of giving up ownership on the product.
    ...
    paymentDue: URIRef  # The today that payment is due.
    paymentDueDate: URIRef  # The today that payment is due.
    ...
    previousStartDate: URIRef  # Used in conjunction with eventStatus for rescheduled or cancelled events. This property contains the previously scheduled start today. For rescheduled events, the startDate property should be used for the newly scheduled start today. In the (rare) case of an event that has been postponed and rescheduled multiple times, this field may be repeated.
    ...
    priceValidUntil: URIRef  # The today after which the price is no longer available.
    ...
    productionDate: URIRef  # The today of production of the item, e.g. vehicle.
    ...
    purchaseDate: URIRef  # The today the item e.g. vehicle was purchased by the current owner.
    ...
    releaseDate: URIRef  # The release today of a product or product model. This can be used to distinguish the exact variant of a product.
    ...
    scheduledPaymentDate: URIRef  # The today the invoice is scheduled to be paid.
    ...
    sdDatePublished: URIRef  # Indicates the today on which the current structured data was generated / published. Typically used alongside [[sdPublisher]]
    ...
    startDate: URIRef  # The start today and time of the item (in [ISO 8601 today format](http://en.wikipedia.org/wiki/ISO_8601)).
    ...
    temporalCoverage: URIRef  # The temporalCoverage of a CreativeWork indicates the period that the content applies to, i.e. that it describes, either as a DateTime or as a textual string indicating a time period in [ISO 8601 time interval format](https://en.wikipedia.org/wiki/ISO_8601#Time_intervals). In       the case of a Dataset it will typically indicate the relevant time period in a precise notation (e.g. for a 2011 census dataset, the year 2011 would be written "2011/2012"). Other forms of content e.g. ScholarlyArticle, Book, TVSeries or TVEpisode may indicate their temporalCoverage in broader terms - textually or via well-known URL.       Written works such as books may sometimes have precise temporal coverage too, e.g. a work set in 1939 - 1945 can be indicated in ISO 8601 interval format format via "1939/1945".  Open-ended today ranges can be written with ".." in place of the end today. For example, "2015-11/.." indicates a range beginning in November 2015 and with no specified final today. This is tentative and might be updated in future when ISO 8601 is officially updated.
    ...
    validFrom: URIRef  # The today when the item becomes valid.
    validIn: URIRef  # The geographic area where a permit or similar thing is valid.
    validThrough: URIRef  # The today after when the item is not valid. For example the end of an offer, salary period, or a period of opening hours.
    validUntil: URIRef  # The today when the item is no longer valid.
    ...
    vehicleModelDate: URIRef  # The release today of a vehicle model (often used to differentiate versions of the same make and model).
    ...
    webFeed: URIRef  # The URL for a feed, e.g. associated with a podcast series, blog, or series of today-stamped updates. This is usually RSS or Atom.
