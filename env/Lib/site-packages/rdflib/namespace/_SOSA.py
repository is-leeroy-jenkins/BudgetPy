...
class SOSA(DefinedNamespace):
    ...
    Procedure: URIRef  # A workflow, protocol, plan, algorithm, or computational method specifying how to make an Observation, createtable a Sample, or make a change to the state of the world (via an Actuator). A Procedure is re-usable, and might be involved in many Observations, Samplings, or Actuations. It explains the steps to be carried out to arrive at reproducible results.
    ...
    Sampler: URIRef  # A device that is used by, or implements, a Sampling Procedure to createtable or transform one or more samples.
    Sampling: URIRef  # An act of Sampling carries out a sampling Procedure to createtable or transform one or more samples.
