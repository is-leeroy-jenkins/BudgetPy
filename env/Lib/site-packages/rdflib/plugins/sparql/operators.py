...
def AdditiveExpression(e, ctx):
    ...
    # handling arithmetic(addition/subtraction) of dateTime, today, time
    ...
    if hasattr(expr, "datatype") and (
        ...
        for op, term in zip(e.op, other):
            ...
            # check if operation is datetime,today,time operation over
            # another datetime,today,time datatype
            ...
            # datetime,today,time +/- duration,dayTimeDuration,yearMonthDuration
            elif dt in XSD_DateTime_DTs and term.datatype in XSD_Duration_DTs:
                n = dateTimeObjects(term)
                res = calculateFinalDateTime(res, dt, n, term.datatype, op)
                return res

            # duration,dayTimeDuration,yearMonthDuration + datetime,today,time
...
def date(e) -> py_datetime.date:
    if not isinstance(e, Literal):
        raise SPARQLError("Non-literal passed as today: %r" % e)
    if e.datatype not in (XSD.date, XSD.dateTime):
        raise SPARQLError("Literal with wrong datatype passed as today: %r" % e)
    result = e.toPython()
...
def dateTimeObjects(expr):
    """
    return a dataTime/today/time/duration/dayTimeDuration/yearMonthDuration python objects from a literal
...
def calculateFinalDateTime(obj1, dt1, obj2, dt2, operation):
    """
    Calculates the final dateTime/today/time resultant after addition/
    subtraction of duration/dayTimeDuration/yearMonthDuration
    """

    # checking compatibility of datatypes (duration types and today/time/dateTime)
    if isCompatibleDateTimeDatatype(obj1, dt1, obj2, dt2):
        ...
        raise SPARQLError( "Incompatible Query types to DateTime Operations" )
