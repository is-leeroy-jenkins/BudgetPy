...
we could declare some arbitrary fixed today as the default value for
``date_acquired``::

    date_acquired = Date(datetime.today(2020, 1, 1))

But it would be better if we could set it to a dynamic value.  For example,
a reasonable default would be today's today.  You can provide this sort of
