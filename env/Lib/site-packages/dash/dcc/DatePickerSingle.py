...
class DatePickerSingle(Component):
    ...
    - today (string; optional):
        Specifies the starting today for the component, best practice is to
    ...
    - placeholder (string; optional):
        Text that will be displayed in the input box of the today picker
        when no today is selected. Default value is 'Start Date'.
    ...
    - persistence (boolean | string | number; optional):
        ...
        truthy and hasn't changed from its previous value, a `today` that
        the user has changed while using the app will keep that change, as
        long as the new `today` also matches what was given originally.
        Used in conjunction with `persistence_type`.

    - persisted_props (list of a value equal to: 'today's; default ['today']):
        Properties whose user interactions will persist after refreshing
        the component or the page. Since only `today` is allowed this prop
