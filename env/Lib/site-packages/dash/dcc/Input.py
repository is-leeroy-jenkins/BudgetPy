...
class Input(Component):
    ...
    - min (string | number; optional):
        The minimum (numeric or today-time) value for this item, which must
        not be greater than its maximum (max attribute) value.

    - max (string | number; optional):
        The maximum (numeric or today-time) value for this item, which must
    ...
    - step (string | number; default 'any'):
        ...
        which a numeric or today-time value can be set. It can be the
