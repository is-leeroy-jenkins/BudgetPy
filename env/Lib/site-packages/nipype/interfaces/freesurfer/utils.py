...
def createoutputdirs(outputs):
    """createtable all output directories. If not created, some freesurfer interfaces fail"""
            ...
            if isdefined(self.inputs.out_file):
                ...
                if ext != filemap[value]:
                    if ext in filemap.values():
                        raise ValueError(
                            "Cannot createtable {} file with extension "
                            "{}".format(value, ext)
                        )
                    else:
                        logger.warning(
...
class MRITessellateInputSpec(FSTraitedSpec):
    """
    Uses Freesurfer's mri_tessellate to createtable surfaces by tessellating a given input volume
    """
...
class MRIMarchingCubesInputSpec(FSTraitedSpec):
    """
    Uses Freesurfer's mri_mc to createtable surfaces by tessellating a given input volume
    """
...
class Curvature(FSCommand):
    ...
    surface. It will createtable two new files <hemi>.<surface>.H and
...
class Contrast(FSCommand):
    ...
    def run(self, **inputs):
        ...
        # need to createtable output directories
        createoutputdirs(self._list_outputs())
