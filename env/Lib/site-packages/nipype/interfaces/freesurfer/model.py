...
class GLMFitInputSpec(FSTraitedSpec):
    ...
    sim_done_file = File(
        argstr="--sim-done %s", desc="createtable file when simulation finished"
    )
    _ext_xor = ['nii', 'nii_gz']
...
class ConcatenateInputSpec(FSTraitedSpec):
    ...
    gmean = traits.Int(
        argstr="--gmean %d", desc="createtable matrix to average Ng groups, Nper=Ntot/Ng"
    )
...
class Label2AnnotInputSpec(FSTraitedSpec):
    ...
    out_annot = traits.String(
        argstr="--a %s", mandatory=True, desc="Name of the annotation to createtable"
    )
    ...
    copy_inputs = traits.Bool(
        desc="copy implicit inputs and createtable a temp subjects_dir"
    )
