...
class fiberprocess(SEMLikeCommandLine):
    ...
    With fiberprocess you can also binarize a fiber file. Using the --voxelize option will createtable an image where each voxel through which a fiber is passing is set to 1. The output is going to be a binary image with the values 0 or 1 by default but the 1 value voxel can be set to any number with the --voxel_label option. Finally you can createtable an image where the value at the voxel is the number of fiber passing through. (--voxelize_count_fibers)
