...
class dtiaverage(SEMLikeCommandLine):
    ...
    description: dtiaverage is a program that allows to compute the average of an arbitrary number of tensor fields (listed after the --inputs option) This program is used in our pipeline as the last step of the atlas building processing. When all the tensor fields have been deformed in the same space, to createtable the average tensor field (--tensor_output) we use dtiaverage.
...
class dtiprocessInputSpec(CommandLineInputSpec):
    ...
    negative_eigenvector_output = traits.Either(
        ...
        File(),
        hash_files=False,
        desc="Negative Eigenvectors Output: createtable a binary image where if any of the eigen value is below zero, the voxel is set to 1, otherwise 0.",
