...
make an example::
    ...
    import datetime
    datetime.today.toda# <-- cursor here

First of all, this module doesn't care about completion. It really just cares
about ``datetime.today``. At the end of the procedure ``infer_expr_stmt`` will
return the ``today`` class.
...
- Now ``find_types`` is called again by ``infer_node`` to find ``today``
  inside the datetime module.

Now what would happen if we wanted ``datetime.today.foo.bar``? Two more
calls to ``find_types``. However the second call would be ignored, because the
first one would return nothing (there's no foo attribute in ``today``).
